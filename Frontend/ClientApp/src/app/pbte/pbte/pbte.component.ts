...
export class PBTEComponent implements OnInit {
  ...
  displayedColumnsExamResultTrainees = ['ClassCode', 'Batch', 'TradeName',
    //'TradeID',
    'TraineeName', 'TraineeCode', 'StatusName', 'ResultStatusName', 'FatherName', 'TraineeCNIC', 'DateOfBirth', 'Education',
    //'TraineeImg',
    'TraineeID', 'PBTEID', 'CollegeID'
  ];
  ...
  @ViewChild('SortNAVTTCExamResultTrainee') SortNAVTTCExamResultTrainee: MatSort;
  @ViewChild('PageNAVTTCExamResultTrainee') PageNAVTTCExamResultTrainee: MatPaginator;
  ...
  getNAVTTCStatsData() {
    this.ComSrv.postJSON('api/PBTE/GetNAVTTCStatsData', this.navttcfilters).subscribe((d: any) => {
  ...
  getPBTETraineeData() {
    this.ComSrv.postJSON('api/PBTE/GetPBTETrainees', this.filters).subscribe((d: any) => {
  ...
  getPBTEDropOutTraineeData() {
    this.ComSrv.postJSON('api/PBTE/GetPBTEDropOutTrainees', this.filters).subscribe((d: any) => {
  ...
  getPBTETradeData() {
    this.ComSrv.postJSON('api/PBTE/GetPBTETrades', this.filters).subscribe((d: any) => {
  ...
  getPBTEExamResultTraineeData() {
    this.ComSrv.postJSON('api/PBTE/GetPBTETraineesExamResult', this.filters).subscribe((d: any) => {
  ...
  getNAVTTCTraineeData() {
    this.ComSrv.postJSON('api/PBTE/GetNAVTTCTrainees', this.navttcfilters).subscribe((d: any) => {
  ...
  getNAVTTCTraineeSqlScriptData() {
    this.ComSrv.postJSON('api/PBTE/GetNAVTTCTraineesSqlScriptData', this.navttcfilters).subscribe((d: any) => {
      //this.navttcTrainees = new MatTableDataSource(d[0]);
      //this.navttcTraineesAfterReset = new MatTableDataSource(d[0]);
  ...
  getNAVTTCTraineeRegisterSqlScriptData() {
    this.ComSrv.postJSON('api/PBTE/GetNAVTTCTraineesRegisterSqlScriptData', this.navttcfilters).subscribe((d: any) => {
      //this.navttcTrainees = new MatTableDataSource(d[0]);
      //this.navttcTraineesAfterReset = new MatTableDataSource(d[0]);
  ...
  getNAVTTCDropOutTraineeData() {
    this.ComSrv.postJSON('api/PBTE/GetNAVTTCDropOutTrainees', this.navttcfilters).subscribe((d: any) => {
    ...
    switch (tableName) {
      case (tableName = "DropOutTrainees"): {
        this.pbteDropOutTrainees.filteredData.forEach(item => {
          ...
          var text = `update dbo.Student set ClassCode='${item.ClassCode}',Batch=${item.Batch},TraineeName='${item.TraineeName}',FatherName='${item.FatherName}',TraineeCNIC='${item.TraineeCNIC}',DateOfBirth='${this._date.transform(item.DateOfBirth, 'dd/MM/yyyy')}',Education='${item.Education}',StudentID=${item.PBTEID},CollegeID=${item.CollegeID},TraineeStatus='${item.StatusName}' where TraineeID=${item.TraineeID},`
      ...
      case (tableName = "Trainees"): {
        this.pbteTrainees.filteredData.forEach(item => {
          ...
          var text = `insert into dbo.Student (ExamId,StudentName,FathersName,DateofBirth,Qualification,Gender,active,Class_Code,NIC,Trainee_ID,Shift,Shift_Time_From,Shift_Time_To) values(${item.ExamID},'${item.TraineeName}','${item.FatherName}','${this._date.transform(item.DateOfBirth, 'dd/MM/yyyy')}','${item.Education}',${item.GenderID},${1},'${item.TraineeCNIC}','${item.TraineeCode}','${item.TraineeShift}','${this._date.transform(item.ShiftFrom, 'hh:mm a')}','${this._date.transform(item.ShiftTo, 'h:mm a')}');`
          //textarray = textarray + text;
          textarray.push(text);
        });
        //console.log(textarrady.join(" "));
      ...
      case (tableName = "Classes"): {
        this.pbteClasses.filteredData.forEach(item => {
          ...
          var text = `insert into table1 (SchemeName,Batch,TSPName,ClassCode,TradeName,TrainingAddressLocation,TehsilName,DistrictName,CertAuthName,TraineesPerClass,GenderName,Duration,StartDate,EndDate,ClassStatusName) values('${item.SchemeName}',${item.Batch},'${item.TSPName}','${item.ClassCode}','${item.TradeName}','${item.TrainingAddressLocation}','${item.TehsilName}','${item.TehsilName}','${item.DistrictName}','${item.CertAuthName}',${item.TraineesPerClass},'${item.GenderName}',${item.Duration},${this._date.transform(item.StartDate, 'dd/MM/yyyy')}','${this._date.transform(item.EndDate, 'dd/MM/yyyy')}','${item.ClassStatusName}');`
          //textarray = textarray + text;
          textarray.push(text);
        });
    ...
    switch (tableName) {
      ...
      case (tableName = "Trainees"): {
        ...
        this.navttcTraineesSqlScriptArray.forEach(item => {
          ...
          //  //console.log(item[key]);
          //});
          var text = `insert into tbl_trainee (traineeid,trainee_name,father_name,gender,disableId,dob,cnic,basic_qualification,pathWayId,mobile,email,address,districtId,instituteId,psdf_traineeId,psdf_ClassCode) values(${traineeid},'${item.TraineeName}','${item.FatherName}','${item.GenderID}','${item.disableId}','${this._date.transform(item.DOB, 'dd/MM/yyyy')}','${item.TraineeCNIC}','${item.Education}','${item.pathWayId}','${item.mobile}','${item.email}','${item.address}','${item.instituteId}','${item.psdf_traineeId}','${item.psdf_ClassCode}');`
          //textarray = textarray + text;
          traineeid++;
          textarray.push(text);
        });
  ...
  generateExcel() {
    ...
    if (this.filters.SchemeID != 0) {
      ...
      //let titleRow = workSheet.addRow(['Unique Trade List & Its Duration (in Months):']);
      workSheet.getCell('A1:C2').value = 'Unique Trade List & Its Duration (in Months):';
      let titleRow = workSheet.getCell('A1:C2');
      titleRow.font = { bold: true, size: 20 };
      titleRow.fill = {
        type: 'pattern',
    ...
    dataForExport.forEach((item, index) => {
      ...
      keys.forEach((key, indx) => {
        if (exportExcel.ImageFieldNames.includes(key) && item[key] != "") {
          let image = workbookTrainee.addImage({ base64: item[key], extension: 'jpeg' })
