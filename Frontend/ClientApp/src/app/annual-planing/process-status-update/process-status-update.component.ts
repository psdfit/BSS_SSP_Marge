...
import { ProcessStatusUpdateDialogComponent } from "src/app/custom-components/process-status-update-dialog/process-status-update-dialog.component";

@Component({
  selector: 'app-process-status-update',
  templateUrl: './process-status-update.component.html',
  styleUrls: ['./process-status-update.component.scss']
})
export class ProcessStatusUpdateComponent implements OnInit {
  ...
  LoadMatTable(tableData: any, tableName: string) {
    ...
    if(tableData.length>0){
      if(tableName=='ProgramData') {
        const excludeColumnArray=["WorkflowRemarks","IsSubmitted","CriteriaRemarks", "StartDate", "EndDate", "StatusRemarks","IsFinalApproved"]
  ...
  OpenDialogue(row) {
    ...
    const dialogRef = this.Dialog.open(ProcessStatusUpdateDialogComponent, {
      width: '40%',
      data: data,
      disableClose: true,
    });
