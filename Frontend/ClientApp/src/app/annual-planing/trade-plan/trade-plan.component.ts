...
export class TradePlanComponent implements OnInit, AfterViewInit {
  ...
  InitTradeDesignInfo() {
    ...
    const clearTradeLotOnChange = (fields: string[]) => {
      fields.forEach(field => {
        this.TradeDesignInfoForm.get(field).valueChanges.subscribe(() => {
          ...
          if (d !== "" && (field === "PerSelectedContraTarget" || field === "ContraTargetThreshold" || field === "Province")) {
    ...
    const fieldsToWatch = ["District", "Cluster", "Province", "ExamCost", "CTM", "PerSelectedContraTarget", "ContraTargetThreshold", "Trade", "TradeLayer"];
  ...
  OnLocationChange() {
    this.Names = []
    this.controlValue = [];
    let selectedShortListControl = this.TradeDesignInfoForm.get("SelectedShortList");
    let selectedCountControl = this.TradeDesignInfoForm.get("SelectedCount");
    this.controlValue = this.TradeDesignInfoForm.get(this.ProgramDesignOn).value;
  ...
  UpdateAutoValue() {
    const d = this.TradeDesignInfoForm.get("PerSelectedContraTarget").value;
    const threshold = (this.TradeDesignInfoForm.get("ContraTargetThreshold").value / 100) * d;
  ...
  getProvinceNames(ids: number[]): string[] {
    return this.ProvinceData.filter(d => ids.includes(d.ProvinceID)).map(d => d.ProvinceName);
  }
  getClusterNames(ids: number[]): string[] {
    return this.ClusterData.filter(d => ids.includes(d.ClusterID)).map(d => d.ClusterName);
  }
  getDistrictNames(ids: number[]): string[] {
    return this.DistrictData.filter(d => ids.includes(d.DistrictID)).map(d => d.DistrictName);
  }
  addTradeLot() {
    ...
    const { CTM, ExamCost, PerSelectedContraTarget } = this.TradeDesignInfoForm.value;
    const { Stipend, bagBadgeCost } = this.selectedProgram[0];
    const { Duration } = this.selectedtradeLayer[0];
    const shortListDistrict = this.TradeDesignInfoForm.get("SelectedShortList").value.split(",");
    this.TradeLotData = shortListDistrict.map(district => this.createTradeLotDetail(district, CTM, Duration, Stipend, bagBadgeCost, ExamCost, PerSelectedContraTarget));
    this.updateTradeLotFormArray();
  }
  createTradeLotDetail(district: string, CTM: number, duration: number, stipend: number, bagBadgeCost: number, examCost: number, perDisContraTarget: number) {
    const totalCost = this.calculateTotalCost(CTM, duration, stipend, bagBadgeCost, examCost, perDisContraTarget);
    return {
      UserID: this.currentUser.UserID,
      ProgramDesignID: 0,
  ...
  //  2*50*2=200
  calculateTotalCost(CTM: number, duration: number, stipend: number, bagBadgeCost: number, examCost: number, perDisContraTarget: number): number {
    return (
  ...
  ChangeTradeLot(index) {
    ...
    tradeLotControl.patchValue(this.createTradeLotDetail(
  ...
  GetCluster(ProvinceId) {
    this.ClusterData = []
    if (ProvinceId == 0) {
      const cluster = this.GetDataObject.cluster.filter(c => this.selectedProgram[0].Cluster.split(",").map(Number).includes(c.ClusterID));
      this.ClusterData = cluster
    } else {
      const cluster = this.GetDataObject.cluster.filter(c => this.selectedProgram[0].Cluster.split(",").map(Number).includes(c.ClusterID));
      this.ClusterData = cluster.filter(c => ProvinceId.includes(c.ProvinceID));
  ...
  GetDistrict(ClusterId) {
    this.DistrictData = []
    if (ClusterId == 0) {
      const district = this.GetDataObject.district.filter(d => this.selectedProgram[0].District.split(",").map(Number).includes(d.DistrictID));
      this.DistrictData = district;
    } else {
      const district = this.GetDataObject.district.filter(d => this.selectedProgram[0].District.split(",").map(Number).includes(d.DistrictID));
      this.DistrictData = district.filter(d => ClusterId.includes(d.ClusterID));
    }
  }
  LoadData() {
    this.ComSrv.postJSON("api/ProgramDesign/LoadData", { UserID: this.currentUser.UserID }).subscribe(
      (response) => {
        this.GetDataObject = response
        this.programDesign = this.GetDataObject.programDesign.filter(d => d.IsInitiate == 0 ||  d.IsInitiate==false)
        ...
        this.LoadMatTable(this.GetDataObject.programWiseBudget, "programWiseBudget");
        this.LoadMatTable(this.GetDataObject.tradeWiseTarget, "tradeWiseTarget");
  ...
  LoadProgramData(programId) {
    ...
    this.selectedProgram = this.GetDataObject.programDesign.filter(p => p.ProgramID === ProgramData);
    this.programBudget = this.GetDataObject.programBudget.filter(p => p.ProgramID === ProgramData);
    this.InitSelectedProgramInfo()
    ...
    this.LoadGender(this.selectedProgram[0].Gender, this.selectedProgram[0].GenderID)
    this.ChangeSchemeDesignOn(this.selectedProgram[0].SchemeDesignOn)
  }
  LoadGender(GenderName: string, GenderID: number) {
    this.GenderData = this.GetDataObject.gender
    if (GenderName != 'Both') {
      this.GenderData = this.GenderData.filter(d => d.GenderName == GenderName)
      this.TradeDesignInfoForm.get('GenderID').setValue(GenderID)
    }
  }
  LoadProvince() {
    this.ProvinceData = []
    const selectedProgramProvince = this.selectedProgram[0].Province.split(",").map(Number)
    this.ProvinceData = this.GetDataObject.province.filter(p => selectedProgramProvince.includes(p.ProvinceID));
  }
  LoadTradeLayer(TradeId) {
    this.tradeLayer = this.GetDataObject.tradeLayer.filter(t => t.TradeID === TradeId);
    this.SelectedTradeInfo.reset()
  }
  LoadTradeLayerData(TradeLayerId) {
    this.selectedtradeLayer = this.GetDataObject.tradeLayer.filter(t => t.TradeDetailMapID === TradeLayerId);
  ...
  async SaveTradePlanInfo() {
    const { ContraTargetThreshold, TraineeContraTarget, TradeLot } = this.TradeDesignInfoForm.value;
    // const TraineeContThreshold = (ContraTargetThreshold / 100) * TraineeContraTarget;
    const totalTraineeContTarget = this.TradeDesignInfoForm.get('TraineeContraTarget').value;
    const totalLotContractedTrainee = this.sumArray(TradeLot.map(d => d.TraineeContTarget));
    if (this.TradeDesignInfoForm.invalid) {
    ...
    if (Number(totalTraineeContTarget)<Number(totalLotContractedTrainee)) {
      this.ComSrv.ShowError(`Contracted target must be equal OR greater than total lots  target.(Contracted Target :${totalTraineeContTarget} | Lots Contracted Target:${totalLotContractedTrainee})`, '', 10000);
      return;
    }
    if (this.TradeLot.length == 0) {
    ...
    if (this.EditCheck == true) {
      const ExistedTradeLot: any[] = this.ExistedTradePlan.filter(d => !this.TradeLots.map(d => d.TradeLotID).includes(d.TradeLotID))
      if (ExistedTradeLot.length > 0) {
        this.OpenDialogue()
        this.ComSrv.ShowError(`Trade Plan already exists for the selected ${this.ProgramDesignOn} value. Please deselect ${this.ProgramDesignOn}.`, '', 10000);
        return
      }
    } else {
    ...
    if (this.TradeDesignInfoForm.valid && this.TradeLot.length > 0) {
      this.TradeDesignInfoForm.get('ProgramDesignOn').setValue(this.ProgramDesignOn)
      this.IsDisabled = true
      this.ComSrv.postJSON("api/ProgramDesign/SaveTradeDesign", this.TradeDesignInfoForm.getRawValue()).subscribe(
        (response) => {
    ...
    if (tableData.length > 0) {
      switch (ReportName) {
        case "tradeWiseTarget":
          this.TradeWiseTableColumns = Object.keys(tableData[0]).filter(key =>
          ...
          break;
        case 'lotWiseTarget':
          this.LotWiseTableColumns = Object.keys(tableData[0]).filter(key =>
            !key.toLowerCase().includes('id')
          );
        ...
        case 'programWiseBudget':
          const excludeColumnArray = ["ClassStartDate", "SSPWorkflow", "IsSubmitted", "TentativeProcessStart", "ProcessStatus", "IsWorkflowAttached", "Workflow", "IsCriteriaAttached", "Criteria", "IsInitiated", "ApprovalStatus", "IsFinalApproval", "WorkflowRemarks", "ProcessStatus", "AssociationStartDate", "AssociationEndDate", "IsWorkflowAttached", "Workflow", "IsCriteriaAttached", "Criteria", "CriteriaRemarks", "StartDate", "EndDate", "StatusRemarks", "IsFinalApproved"]
          this.ProgramWiseTableColumns = Object.keys(tableData[0])
  ...
  async EditTradePlan(row: any) {
    debugger;
    const GenderID = this.GenderData.filter(g => g.GenderName == row.Gender).map(g => g.GenderID)
    this.EditCheck = true;
    const selectedTradeDesign = await this.TradeDesign.filter(d => d.TradeDesignID == row.TradeDesignID)
    const programIsInitiated: any[] = this.GetDataObject.programDesign.filter((d: any) =>  d.ProgramID == selectedTradeDesign[0].Scheme && (d.IsInitiate == 1 || d.IsInitiate==true)
    );
    if (programIsInitiated.length > 0) {
      this.IsInitiated = true
      this.programDesign = this.GetDataObject.programDesign.filter(d => d.IsInitiate == 1 || d.IsInitiate==true)
    ...
    this.TradeDesignInfoForm.get("Scheme").setValue(selectedTradeDesign[0].Scheme)
    switch (selectedTradeDesign[0].ProgramDesignOn) {
      case "Province":
        this.TradeDesignInfoForm.get("Province").setValue(selectedTradeDesign[0].ProvinceID.split(',').map(Number))
        break;
      case "Cluster":
        this.TradeDesignInfoForm.get("Province").setValue(selectedTradeDesign[0].ProvinceID.split(',').map(Number))
        this.GetCluster(selectedTradeDesign[0].ProvinceID.split(',').map(Number))
        this.TradeDesignInfoForm.get("Cluster").setValue(selectedTradeDesign[0].ClusterID.split(',').map(Number))
        break;
      case "District":
        this.TradeDesignInfoForm.get("Province").setValue(selectedTradeDesign[0].ProvinceID.split(',').map(Number))
        this.GetCluster(selectedTradeDesign[0].ProvinceID.split(',').map(Number))
        this.TradeDesignInfoForm.get("Cluster").setValue(selectedTradeDesign[0].ClusterID.split(',').map(Number))
        this.GetDistrict(selectedTradeDesign[0].ClusterID.split(',').map(Number))
        this.TradeDesignInfoForm.get("District").setValue(selectedTradeDesign[0].DistrictID.split(',').map(Number))
        break;
    }
    ...
    this.TradeLot.controls.forEach((control: FormGroup, index: number) => {
      control.controls.TraineeContTarget.setValue(this.TradeLots[index].TraineeSelectedContTarget)
  ...
  async CheckTradePlanExisted(form: FormGroup) {
    ...
    await Promise.all(this.controlValue.map(async (LocationID) => {
      this.paramObject = {
