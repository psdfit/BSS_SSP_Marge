...
export class ProgramInitiateComponent implements OnInit {
  ...
  LoadData() {
    this.ComSrv.postJSON("api/ProgramDesign/LoadSchemeData", {
      ...
      (response) => {
        ...
        this.programDesign = this.GetDataObject.programDesignSummary.filter(
          (p) => p.IsInitiated == false && p.IsSubmitted == true
        );
        const program = this.GetDataObject.programDesignSummary.filter(
  ...
  LoadProgramData(programId) {
    this.programData = this.GetDataObject.programDesignSummary.filter(
      (p) => p.ProgramID === programId && p.IsInitiated === false
    );
  ...
  Save() {
    if (this.SchemeInfoForm.valid) {
      ...
      this.ComSrv.postJSON(
        "api/ProgramDesign/UpdateSchemeInitialization",
        this.SchemeInfoForm.getRawValue()
      ).subscribe(
        (response) => {
          ...
          this.ComSrv.openSnackBar(
            "Scheme Initialized for Approval.",
            "Close",
            5000
          );
      ...
      switch (ReportName) {
        case "Program":
          const excludeColumnArray = [
            ...
            "EndDate",
            "StatusRemarks",
            "IsFinalApproved","ProgramName","SupportItems","ClassStartDate","PlaningType","SelectionMethod"
          ];
