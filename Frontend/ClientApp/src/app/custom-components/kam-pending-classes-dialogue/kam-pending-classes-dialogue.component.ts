...
import { EnumUserLevel, EnumBusinessRuleTypes, EnumExcelReportType } from '../../shared/Enumerations';
...
@Component({
  selector: 'app-kam-pending-classes-dialogue',
  templateUrl: './kam-pending-classes-dialogue.component.html',
  styleUrls: ['./kam-pending-classes-dialogue.component.scss'],
  providers: [ DatePipe]
})
export class KAMPendingClassesDialogueComponent implements OnInit {
 ...
 ClasssesListForExcel: any[];
  ...
  constructor(private http: CommonSrvService, public dialog: MatDialog, private dialogue: DialogueService, public dialogRef: MatDialogRef<KAMPendingClassesDialogueComponent>, private router: Router,private _date: DatePipe,
    @Inject(MAT_DIALOG_DATA) public data: any) { }

  ngOnInit(): void {
    this.filters.UserID = this.data.UserID;
    this.filters.TSPID = this.data.TSPID;
  ...
  getPendingClasses() {
    ...
    if (this.data.TileName == "KAMDashboardpendingclasses") {
      this.title = "Pending Employments Classes";
      this.ExportButtonHideShow=true;
      this.GetPendingEmploymentsClassesByKAM(this.filters.UserID);
      this.displayedColumnsClass = [
        'TSPName','TSPCode', 'ClassCode','StartDate', 'EndDate','ClassStatusName'];
  ...
  GetPendingEmploymentsClassesByKAM(UserID) {
    this.http.getJSON(`api/KAMDashboard/GetPendingEmploymentsClassesByKAM?KAMUserID=`+ UserID)
      .subscribe(
        (d: any) => {

          this.ClasssesListForExcel=d[0];
          this.classesArray = new MatTableDataSource(d[0]);
  ...
  getPendingRTPsClassData() {
    this.http.postJSON(`api/Class/FetchPendingRTPClassesByKAMUser`, this.filters)
      .subscribe(
        (d: any) => {
  ...
  getPendingRegisterationsClassData() {
    this.http.postJSON(`api/Class/FetchPendingRegistertionClassesByKAMUser`, this.filters)
      .subscribe(
        (d: any) => {
          //  this.Class = new MatTableDataSource(d);
          this.classesArray = new MatTableDataSource(d);
  ...
  getPendingInceptionReportClassData() {
    this.http.postJSON(`api/Class/FetchPendingInceptionReportClassesByKAMUser`, this.filters)
      .subscribe(
        (d: any) => {
          //  this.Class = new MatTableDataSource(d);
          this.classesArray = new MatTableDataSource(d);
  ...
  exportToExcel() {
    debugger;
    let filteredData = [...this.ClasssesListForExcel]
    let data = {
    };

    let exportExcel: ExportExcel = {
      Title: 'Pending Employments Classes',
      Author: '',
      Type: EnumExcelReportType.PendingClassesinKAMDashboard,
      Data: data,
      List1: this.populateDataPendingEmploymentsClasses(filteredData),
    };
    this.dialogue.openExportConfirmDialogue(exportExcel).subscribe();
  }


  populateDataPendingEmploymentsClasses(data: any) {
    return data.map(item => {
      return {
        "TSP Name": item.TSPName
        ,"TSP Code": item.TSPCode
