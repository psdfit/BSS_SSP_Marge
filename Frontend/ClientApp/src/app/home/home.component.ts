...
export class HomeComponent implements OnInit, AfterViewInit {
  ...
  displayedColumnsClass = ['sn', 'InternalUserAction', 'ClassCode', 'Duration', 'ClassStatusName', 'StartDate', 'EndDate', 'TrainingAddressLocation', 'TradeName', 'GenderName', 'TraineesPerClass', 'TehsilName', 'CertAuthName', 'InceptionReportDueOn', 'TraineeRegistrationDueOn'];
  displayedColumnsScheme = ['SchemeName', 'SchemeCode', 'PTypeName', 'PCategoryName', 'FundingSourceName', 'GenderName', 'MinAge', 'MaxAge', 'StipendMode', 'BusinessRuleType'];
  displayedColumnsTSP = ['TSPName', 'Address', 'TSPCode', 'SchemeName', 'NTN', 'PNTN', 'GST', 'FTN', 'HeadName', 'CPName', 'CPLandline'];
  displayedColumnsTrainee = ['sn', 'ClassCode', 'TraineeName', 'TraineeCode', 'ClassBatch', 'StatusName', 'TraineeCNIC', 'FatherName', 'GuardianName', 'GenderName', 'TraineeAge', 'ContactNumber1', 'TraineeEmail', 'PermanentResidence', 'PermanentDistrictName', 'PermanentTehsilName', 'TraineeDoc'];
  ...
  @ViewChild(MatPaginator, { static: false }) paginator: QueryList<MatPaginator>;
  ...
  constructor(private ComSRV: CommonSrvService, public dialog: MatDialog, private router: Router, public dialogueService: DialogueService) {
  ...
  ngOnInit() {
    this.ComSRV.OID.subscribe(OID => {
      ...
      if (this.currentUser.UserLevel == this.enumUserLevel.TSP) {
        ...
        this.displayedColumnsClass = ['sn', 'Action', 'RTP', 'ClassCode', 'Duration', 'ClassStatusName', 'StartDate', 'EndDate', 'TrainingAddressLocation', 'TradeName', 'GenderName', 'TraineesPerClass', 'TehsilName', 'CertAuthName', 'Batch', 'DistrictName', 'InceptionReportDueOn', 'TraineeRegistrationDueOn', 'NTPStatus'];
  ...
  CheckPasswordAge() {
    ...
    this.ComSRV.postJSON('api/Users/CheckPasswordAge', this.user).subscribe((response: any) => {
      if (response == "1") {
        this.ComSRV.ShowError("Your session will expire in 180 Seconds.Please change your password", 'Close', 15000);
  ...
  logout() {
    this.ComSRV.getJSON('api/Users/Logout?SessionID=' + sessionStorage.getItem("SessionID") + '&UserID=' + sessionStorage.getItem("UserID")).subscribe((d: any) => {
  ...
  openDialogChangePass(): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result != undefined) {
        ...
        this.ComSRV.postJSON('api/Users/ChangePassword', { UserID: this.ComSRV.getUserDetails().UserID, UserPassword: a, InActive: false })
          .subscribe((d: any) => {
            if (d == true) {
              this.ComSRV.openSnackBar(environment.UpdateMSG.replace('${Name}', 'Change Password'));
            } else {
              this.ComSRV.ShowError('Given Password is Already existed.Please use another password', 'Close');
            }
            // this.reset(r);
          },
            error => this.error = error // error path
  ...
  getUnverifiedTraineeEmail(UserId: number) {
    ...
    this.ComSRV.getJSON(`api/Scheme/${Url}`)
      .subscribe(data => {
  ...
  getFilteredStatsData() {
    this.ComSRV.postJSON(`api/HomeStats/FetchHomeStatsByUser`, this.filters)
      .subscribe((data: any) => {
        console.log(data)
  ...
  getFilteredStatsDataPendingClasses() {
    this.ComSRV.postJSON(`api/HomeStats/FetchPendingHomeStatsByUser`, this.filters)
      .subscribe((data: any) => {
  ...
  toggleRTP(row) {
    this.ComSRV.confirm().subscribe(result => {
      if (result) {
        this.ComSRV.postJSON('api/Class/RTP', { 'ClassID': row.ClassID, 'RTP': row.RTP })
          .subscribe((d: any) => {
            this.ComSRV.openSnackBar(row.RTP == true ? environment.ActiveRTP.replace("${Name}", this.EnText) : environment.RemoveRTP.replace("${Name}", this.EnText));
  ...
  async UnverifiedTraineeEmailAddress() {
    ...
    const Param = this.GetParamString('RD_UnverifiedTraineeEmail', { tspId: this.currentUser.UserID, KamId: 0 });
  ...
  GetTraineeDraftData() {
    ...
    this.ComSRV.getJSON(`api/TraineeProfile/${Url}`)
      .subscribe(
        (response: any) => {
  ...
  openDialog(row): void {
    this.ComSRV.postJSON('api/RTP/RD_RTPClassDataBy', { "ClassID": row.ClassID }).subscribe((response: any) => {
  ...
  routeToRegistration(row: any) {
    ...
    this.ComSRV.getJSON(`api/InceptionReport/GetInceptionReportByClass?classID=${row.ClassID}`)
      .subscribe(data => {
        ...
        if (data[0].length > 0) {
          ...
          this.ComSRV.ShowError("Please submit inception report first, before registration.");
        }
      }, error => {
        this.error = error;
      });
  ...
  checkTSPColor(classStatusid: number) {
    this.ComSRV.postJSON('api/TSPColor/RD_TSPMasterColorByID', { 'CurUserID': this.currentUser.UserID }).subscribe((response: any) => {
      if (response[0].TSPColorID == EnumTSPColorType.Red && classStatusid != EnumClassStatus.Active) {
        this.ComSRV.ShowError("TSPs with color red can only handle active classes");
        this.blacklistedTSPwithRed = true;
        return;
      }
      if (response[0].TSPColorID == EnumTSPColorType.Black) {
        this.ComSRV.ShowError("BlackListed TSPs are not allowed to perform any action");
  ...
  routeToInceptionReport(row: any) {
    ...
    this.ComSRV.postJSON('api/InceptionReport/RD_InceptionReportBy', { "ClassID": row.ClassID }).subscribe((response: any) => {
  ...
  buildDataSource() {
    ...
    let classData = this.groupBy('ClassStatusName', this.PageClassData, this.reducedGroups)
    let index = 0;
    classData = classData.map((item) => { if (!item.isGroup) { return { ...item, Sr: ++index } } else { return item } })
    this.Class = new MatTableDataSource(classData);
  }
  ...
  initSchemePagedData() {
    ...
    merge(this.SortScheme.sortChange, this.PageScheme.page, this.schemeFilter.valueChanges, this.tspFilter.valueChanges, this.classFilter.valueChanges).pipe(
      startWith({}),
      switchMap(() => {
        let pagedModel = {
  ...
  getSchemePagedData(pagingModel, filterModel) {
    return this.ComSRV.postJSON('api/Scheme/FetchSchemeByUserPaged', { pagingModel, filterModel });
  }
  initTSPPagedData() {
    this.SortTSP.sortChange.subscribe(() => this.PageTSP.pageIndex = 0);
    this.PageTSP.pageSize = 10;
    merge(this.SortTSP.sortChange, this.PageTSP.page, this.schemeFilter.valueChanges, this.tspFilter.valueChanges, this.classFilter.valueChanges).pipe(
      startWith({}),
      switchMap(() => {
        let pagedModel = {
  ...
  getTSPPagedData(pagingModel, filterModel) {
    return this.ComSRV.postJSON('api/TSPDetail/FetchTSPByUserPaged', { pagingModel, filterModel });
  }
  initClassPagedData() {
    this.SortClass.sortChange.subscribe(() => this.PageClass.pageIndex = 0);
    this.PageClass.pageSize = 10;
    merge(this.SortClass.sortChange, this.PageClass.page, this.schemeFilter.valueChanges, this.tspFilter.valueChanges, this.classFilter.valueChanges).pipe(
      startWith({}),
      switchMap(() => {
        let pagedModel = {
  ...
  getClassPagedData(pagingModel, filterModel) {
    return this.ComSRV.postJSON('api/Class/FetchClassesByUserPaged', { pagingModel, filterModel });
  }
  initTraineePagedData() {
    this.SortTrainee.sortChange.subscribe(() => this.PageTrainee.pageIndex = 0);
    this.PageTrainee.pageSize = 10;
    merge(this.SortTrainee.sortChange, this.PageTrainee.page, this.schemeFilter.valueChanges, this.tspFilter.valueChanges, this.classFilter.valueChanges).pipe(
      startWith({}),
      switchMap(() => {
        let pagedModel = {
  ...
  getTraineePagedData(pagingModel, filterModel) {
    return this.ComSRV.postJSON('api/TraineeProfile/FetchTraineesByUserPaged', { pagingModel, filterModel });
  }
  ...
  GetData() {
    this.ComSRV.getJSON('api/Tehsil/GetTehsil').subscribe((d: any) => {
      this.District = d[1];
  ...
  getCluster() {
    this.ComSRV.getJSON('api/Cluster/GetCluster').subscribe((d: any) => {
      this.Clustor = d;
    }, error => this.error = error // error path
    );
  }
  getSector() {
    this.ComSRV.getJSON('api/Sector/GetSector').subscribe((d: any) => {
      this.Sector = d;
    }, error => this.error = error // error path
    );
  }
    ...
    for (var i = 0; i < markerArray.length; i++) {
      ...
      if (mk) {
        if (typeof markerArray[i].ClassCode != 'undefined') {
          ...
          cclass += 1;
          mk._popup._content += "<div>Total Classes: " + cclass + "</div><div>Male: " + cmale + ", Female: " + cfemale + ", Both: " + cboth + "</div><table class='table' style='border-bottom:2px solid black;'>"
        ...
        cclass = 1;
        let popupText = "<div>Total Classes: " + cclass + "</div><div>Male: " + cmale + ", Female: " + cfemale + ", Both: " + cboth + "</div><table class='table' style='border-bottom:2px solid black;'>"
          + "<tr><td>Class Code</td><td>" + markerArray[i].ClassCode + "</td></tr>"
          + "<tr><td>TSP Name</td><td>" + markerArray[i].TSPName + "</td></tr>"
          + "<tr><td>Cluster Name</td><td>" + markerArray[i].ClusterName + "</td></tr>"
          // + "<tr><td>Gender Name</td><td>"+markerArray[i].GenderName+"</td></tr>"
  ...
  getClassDataGis() {
    this.ComSRV.getJSON('api/Class/GetClass').subscribe((d: any) => {
