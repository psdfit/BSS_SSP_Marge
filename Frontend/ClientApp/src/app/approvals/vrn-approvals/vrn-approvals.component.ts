...
@Component({
  selector: 'app-vrn-approvals',
  templateUrl: './vrn-approvals.component.html',
  styleUrls: ['./vrn-approvals.component.scss'],
  providers: [
...
export class VrnApprovalsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.setTitle("Voucher Recommendation Note");
  ...
  GetSRN() {
    ...
    this.http.postJSON(`api/SRN/GetVRN`, { Month: this.month.value, KAMID: this.filters.KAMID, SchemeID: this.filters.SchemeID, TSPMasterID: this.filters.TSPMasterID }).subscribe(
      (data: any) => {
        console.log(data)
