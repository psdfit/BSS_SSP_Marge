...
export class AnnualPlanApprovalComponent implements OnInit {
  ...
  LoadData() {
    this.ComSrv.postJSON("api/ProgramDesign/LoadSchemeData", { UserID: this.currentUser.UserID }).subscribe(
      (response) => {
        ...
        const ProgramTablesData = this.GetDataObject.programDesignSummary.filter(d => d.IsInitiated == true && ( d.IsFinalApproved ==true || d.IsFinalApproved ==false ) );
  ...
  LoadMatTable(tableData: any, dataType: string) {
    ...
    switch (dataType) {
      case 'Program':
        const excludeColumnArray=["WorkflowRemarks","SSPWorkflow","IsSubmitted","ProcessStatus", "AssociationStartDate", "AssociationEndDate","IsWorkflowAttached","Workflow","IsCriteriaAttached", "Criteria","CriteriaRemarks", "StartDate", "EndDate", "StatusRemarks","IsFinalApproved"]
