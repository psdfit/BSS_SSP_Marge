...
@Component({
  selector: 'app-cr-verified-trainee-approvals',
  templateUrl: './cr-verified-trainee-approvals.component.html',
  styleUrls: ['./cr-verified-trainee-approvals.component.scss'],
  providers: [GroupByPipe, DatePipe]
})
export class VerifiedTraineeChangeRequestApprovalsComponent implements OnInit {
  ...
  filters: ICRVerifiedTraineeListFilter = { SchemeID: 0, ClassID: 0, TSPID: 0, UserID: 0 };
  ...
  ngOnInit(): void {
    this.http.setTitle("Verified Trainee Change Request");
    this.title = "";
  ...
  GetSubmittedTrainees() {
    this.http.postJSON('api/TraineeChangeRequest/GetTraineeChangeRequest', { Process_Key: "CR_TRAINEE_VERIFIED", UserID: this.userid, OID: this.http.OID.value, SchemeID: this.filters.SchemeID, TSPID: this.filters.TSPID, ClassID: this.filters.ClassID }).subscribe((d: any) => {
      this.trainees = d[0];
  ...
  exportToExcel(name?: string) {
    ...
    let exportExcel: ExportExcel = {
      Title: 'Verified Trainees Change Request Report',
  ...
  openApprovalDialogue(row: any): void {
    let processKey = EnumApprovalProcess.CR_TRAINEE_VERIFIED;
...
export interface ICRVerifiedTraineeListFilter {
  SchemeID: number;
  TSPID: number;
  ClassID: number;
  UserID: number;
