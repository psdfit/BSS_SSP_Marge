...
@Component({
  selector: 'app-initiate-association',
  templateUrl: './initiate-association.component.html',
  styleUrls: ['./initiate-association.component.scss']
})
export class InitiateAssociationComponent implements OnInit {
  ...
  LoadMatTable(tableData: any[], ReportName: string) {
    ...
    if (tableData.length > 0) {
      if (ReportName == 'ProgramData') {
        const excludeColumnArray = ["WorkflowRemarks", "CriteriaRemarks", "StatusRemarks", "IsFinalApproved", "IsSubmitted"]
