...
export class AssociationSubmissionComponent implements OnInit {
  ...
  InitAssociationForm() {
    ...
    this.AssociationForm.get("TrainingLocation").valueChanges.subscribe(
      (Id) => {
        this.AssociationForm.get("TradeLot").setValue("");
        this.AssociationForm.get("TrainerDetailID").setValue("");
        this.TrainerProfile = [];
        const TrainingDistrict = this.trainingLocation.filter(
          (d) => d.TrainingLocationID === Id
        );
    ...
    this.AssociationForm.get("TradeLot").valueChanges.subscribe((Id) => {
      ...
      if (this.TradeLot != undefined) {
        ...
        const TSPAssociationSubmission: any[] =
          this.TSPAssociationSubmission.filter(
            (d) =>
              d.TradeLot == Id &&
  ...
  async updateTrainerData(ParentTrade: any) {
    ...
    if (this.TrainerProfile.length == 0) {
      this.ComSrv.ShowError(
        "system couldn't find a trainer for selected trade lot."
      );
    }
  }
  Save() {
    if (this.IsExistedTradeLot == true) {
      this.ComSrv.ShowError(
        "TradeLot is already existed with this selected location"
      );
      return;
    }
  ...
  async GetAssociationSubmission() {
    ...
    this.TSPAssociationSubmission = await this.FetchData(
      this.SPName,
      this.paramObject
    );
  ...
  async GetCriteriaMainCategory(CriteriaHeaderID) {
    ...
    this.CriteriaMainCategory = await this.FetchData(
      this.SPName,
      this.paramObject
    );
  ...
  async GetCriteriaSubCategory(CriteriaHeaderID) {
    ...
    this.CriteriaSubCategory = await this.FetchData(
      this.SPName,
      this.paramObject
    );
    this.PopulateAssociationDetail(this.CriteriaMainCategory);
  ...
  async GetTradeLot(DistrictID, ProgramID) {
    ...
    try {
      ...
      if (this.TradeLot.length > 0) {
        ...
        this.ComSrv.ShowError(
          "No Trade Lots found for the specified criteria."
        );
      }
    } catch (error) {
  ...
  async FetchData(SPName: string, paramObject: any) {
    try {
      ...
      const data = await this.ComSrv.getJSON(
  ...
  getItemsByCategory(category: string): any[] {
    return this.associationDetail.controls
      .filter((control: FormGroup) => {
  ...
  async Edit(row: any) {
    ...
    this.AssociationForm.get("TspAssociationMasterID").setValue(
      row.TspAssociationMasterID
    );
    await this.TSPAssociationDetail(row.TspAssociationMasterID);
    await this.PopulateAssociationDetail(this.AssociationData);
    await this.AssociationForm.get("TrainerDetailID").setValue(
      row.TrainerDetailID
    );
  }
