...
export class BaseDataComponent implements OnInit {
  ...
  onChangeRegistration() {
    ...
    const _registrationAuthority = this.Certificate.get(
      "RegistrationAuthority"
    ).value;
    const _formControl = this.Certificate.controls;

    if (_registrationAuthority == 5) {
      _formControl.IssuanceDate.clearValidators();
      _formControl.ExpiryDate.clearValidators();
    } else {
      _formControl.IssuanceDate.setValidators(Validators.required);
      _formControl.ExpiryDate.setValidators(Validators.required);
    }
    _formControl.IssuanceDate.updateValueAndValidity();
    _formControl.ExpiryDate.updateValueAndValidity();
  }
  ...
  InitBankInfoForm() {
    ...
    this.BankInfoForm.get("BankName")
      .valueChanges.pipe(takeUntil(this.ngUnsubscribe))
      .subscribe((bankId) => {
        if (bankId != null) {
          const selectedOption = this.bank.find(
            (s) => s.BankID === bankId
          ).BankName;
          if (selectedOption == "Other") {
            this.BankInfoForm.get("OtherBank").setValidators(
              Validators.required
            );
            this.readOnly = false;
            this.required = true;
  ...
  EditTrainingLocation(data: any) {
    this.EditCheck = true;
    this.savebtn = "Update ";
    this.TrainingForm.get("TrainingLocationID").setValue(
  ...
  EditCertificationDetail(data: any) {
    debugger;
    const CheckCertificateMapped = this.TradeMapping.filter(
      (d) => d.TrainingCertificationID == data.TrainingCertificationID
    );
    if (CheckCertificateMapped.length > 0) {
      this.ComSrv.ShowError(
        "This certificate is associated with an accepted trade and cannot be edited."
      );
      return;
    }
    this.savebtn = "Update ";
    this.Certificate.get("TrainingCertificationID").setValue(
      data.TrainingCertificationID
    );
  ...
  SaveTrainingInfo() {
    const TrainingLocation = this.TrainingForm.get(
      "TrainingLocationName"
    ).value;
    if (this.TrainingLocation.length > 0 && this.EditCheck != true) {
      const IsExisted: any[] = this.TrainingLocation.filter(
        (d) =>
          d.TrainingLocationName.trim().toLowerCase() ==
          TrainingLocation.trim().toLowerCase()
      );
      if (IsExisted.length > 0) {
        this.ComSrv.ShowError(
          "The training location already exists. Please choose a different training location name"
        );
        return;
      }
    }
    if (this.TrainingForm.valid) {
      this.ComSrv.postJSON(
        "api/BaseData/SaveTrainingLocation",
  ...
  SaveCertificateInfo() {
    if (this.Certificate.valid) {
      const RegistrationStatus =
        this.Certificate.controls.RegistrationStatus.value;
      ...
      this.ComSrv.postJSON(
        "api/BaseData/SaveCertification",
  ...
  async SaveTradeInfo() {
    if (this.TradeForm.valid) {
      await this.ComSrv.postJSON(
        "api/BaseData/SaveTradeMapping",
  ...
  SaveTrainerInfo() {
    ...
    if (TrainerCNIC != "" && this.TrainerEditCheck != true) {
      const IsExisted: any[] = this.TrainerProfile.filter(
        (d) => d.TrainerCNIC == TrainerCNIC
      );
      if (IsExisted.length > 0) {
        this.ComSrv.ShowError(
          "Trainer CNIC already exists. Please choose a different CNIC"
        );
        return;
      }
    }
    if (this.trainerDetails.length > 0) {
      if (this.TrainerForm.valid && this.trainerDetails.valid) {
        this.ComSrv.postJSON(
          "api/BaseData/SaveTrainerProfile",
          this.TrainerForm.value
        ).subscribe(
          (response) => {
  ...
  FormValueComparison(formValue: any, firstControl: any, secondControl: any) {
    ...
    if (firstValue && secondValue && firstValue === secondValue) {
      this.BankInfoForm.controls[secondControl].setErrors({
        customError: `${
          firstControl.charAt(0).toUpperCase() + firstControl.slice(1)
        } and ${
          secondControl.charAt(0).toUpperCase() + secondControl.slice(1)
  ...
  GetTehsil(tehsilID) {
    if (tehsilID > 0) {
      const tehsilData = this.GetDataObject.tehsil.find(
        (t) => t.TehsilID == tehsilID
      );
      const districtData = this.GetDataObject.district.find(
        (t) => t.DistrictID == tehsilData.DistrictID
      );
      this.DistrictData = this.GetDataObject.district.filter(
        (d) => d.DistrictID == districtData.DistrictID
      );
      this.ClusterData = this.GetDataObject.cluster.filter(
        (d) => d.ClusterID == districtData.ClusterID
      );
      this.ProvinceData = this.GetDataObject.province.filter(
        (d) => d.ProvinceID == districtData.ProvinceID
      );
  ...
  GetData() {
    ...
    this.ComSrv.postJSON("api/BaseData/GetData", {
      ...
      (response) => {
        ...
        this.Gender = this.GetDataObject.Gender.filter(
          (d) => d.GenderID != 7 && d.GenderID != 8
        );
  ...
  OpenMapDialogue() {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      this.TrainingForm.get("GeoTagging").setValue(
        this.ComSrv.sharedDataObj.join(",")
      );
      this.readOnly = false;
    });
  }
  ...
  LoadMatTable(tableData: any, ReportName: string) {
    if (tableData != undefined) {
      switch (ReportName) {
        case "BankDetail":
          this.BankTableColumns = Object.keys(tableData[0]).filter(
            (key) =>
        ...
        case "TrainingLocation":
          this.TrainingTableColumns = Object.keys(tableData[0]).filter(
            (key) =>
              !key.toLowerCase().includes("id") &&
          ...
          break;
        case "Certification":
          this.CertificateTableColumns = Object.keys(tableData[0]).filter(
            (key) =>
              !key.includes("ID") &&
              ![
          ...
          break;
        case "TradeMapping":
          this.TradeMapTableColumns = Object.keys(tableData[0]).filter(
            (key) => !key.includes("ID") && ![""].includes(key)
          );
          ...
          break;
        case "TrainerProfile":
          this.TrainerTableColumns = Object.keys(tableData[0]).filter(
            (key) =>
              !key.includes("ID") &&
              ![
                "TrainerCV",
  ...
  async proceedRegistrationPayment() {
    const bankDetail = this.TSPProfileScore.some(
      (score) => score.FormName === "BankDetail" && score.Score === 0
    );
    if (bankDetail) {
  ...
  async getTrainerDetail() {
    ...
    this.TrainerDetailByUserID = await this.FetchData(
      this.SPName,
      this.paramObject
    );
  }
  async checkTradeTrainerMapped() {
    ...
    if (this.TspMappedTrades.length != 0) {
      const trainerTradeIDs = this.TrainerDetailByUserID.map(
        (item) => item.TrainerTradeID
      );
      const tradeNotMappedTrainer: any[] = this.TspMappedTrades.filter(
        (item) => !trainerTradeIDs.includes(item.TradeID)
      ...
      this.ComSrv.ShowError(
        "Trade mapping with training location is necessary to set up the trainer profile.",
        "Close",
        10000
      );
    }
  ...
  calculateScore() {
    this.currentScore = this.TSPProfileScore.map((d) => d.Score).reduce(
      (accumulator, currentValue) => accumulator + currentValue,
      0
    );
    this.PendingForm = this.TSPProfileScore.filter((d) => d.Score == 0).map(
      (d) => d.FormName
    );
  }
  ...
  async GetTSPProfileScore() {
    ...
    this.TSPProfileScore = await this.FetchReport(
      this.SPName,
      this.paramObject
    );
    this.calculateScore();
  ...
  checkProfileScores() {
    const businessProfileIncomplete = this.TSPProfileScore.some(
      (score) => score.FormName === "BusinessProfile" && score.Score === 0
    );
    const contactPersonIncomplete = this.TSPProfileScore.some(
      (score) => score.FormName === "ContactPerson" && score.Score === 0
    );
  ...
  async FetchReport(SPName: string, paramObject: any) {
    try {
      const Param = this.GetParamString(SPName, paramObject);
      const data: any = await this.ComSrv.postJSON(
