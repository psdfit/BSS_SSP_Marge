...
export class DeviceRegistrationComponent implements OnInit {
  ...
  fetchTSPLocationsByClass(classId: number): void {
    this.ComSrv.getJSON(
      `api/DeviceManagement/GetTSPDetailsByClassID?ClassID=${classId}`
    ).subscribe(
      (locations: string[]) => {
        if(locations) 
        this.TSPLocations = locations; // Populate TSP locations
        else this.ComSrv.ShowWarning("No TSP locations found for the selected class", "Close");
    
      },
      (error) => {
        this.ComSrv.ShowError("Error fetching TSP locations:");
      }
    );
  }
  getDependantFilters() {
