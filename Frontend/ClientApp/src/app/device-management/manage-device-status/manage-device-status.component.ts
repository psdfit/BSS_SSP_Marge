...
export class ManageDeviceStatusComponent implements OnInit {
  ...
  getTSPDetailByScheme(schemeId: number) {
    ...
    this.ComSrv.getJSON(
      `api/Dashboard/FetchTSPsByScheme?SchemeID=${schemeId}`
    ).subscribe(
      (data) => {
  ...
  getClassesByTsp(tspId: number) {
    this.classFilter.setValue(0);
    this.ComSrv.getJSON(
      `api/Dashboard/FetchClassesByTSP?TspID=${tspId}`
    ).subscribe(
      (data) => {
  ...
  getClassesBySchemeFilter() {
    ...
    this.ComSrv.getJSON(
      `api/Dashboard/FetchClassesBySchemeUser?SchemeID=${this.schemeFilter.value}&UserID=${this.currentUser.UserID}`
    ).subscribe(
      (data) => {
        this.classesArray = data as any[];
      },
  ...
  getSchemesData() {
    this.ComSrv.getJSON(
      `api/TSRLiveData/GetSchemesForGSR?OID=${this.ComSrv.OID.value}`
    ).subscribe(
      (d: any) => {
  ...
  getTspDetails() {
    this.ComSrv.getJSON(`api/Dashboard/FetchTSPDetails`).subscribe(
      (data) => {
  ...
  SaveFormData() {
    ...
    if (this.DeviceRegistrationForm.valid) {
      this.http
        .postJSON(
          "api/DeviceManagement/Save",
          this.DeviceRegistrationForm.getRawValue()
        )
        .subscribe(
          (response: any[]) => {
      ...
      this.ComSrv.ShowError(
        "All fields marked with * are required. Please complete these fields before submitting the form."
      );
    }
  }
  ...
  LoadMatTable(tableData: any[]) {
    ...
    if (tableData.length > 0) {
      ...
      const activeDevices = tableData.filter(
        (device) => device.DeviceStatus === true
      );
      const inactiveDevices = tableData.filter(
        (device) => device.DeviceStatus === false
      );
