...
@Component({
  selector: "app-medial-cost-doc-dialog",
  templateUrl: "./medial-cost-doc-dialog.component.html",
  styleUrls: ["./medial-cost-doc-dialog.component.scss"],
})
export class MedicalCostDocDialogComponent implements OnInit {
  currentUser: any = {}
  ...
  private readonly UPLOAD_URL = "api/IPDocsVerification/UploadMedicalCostDocs"; // Replace with actual URL
  private readonly FETCH_DOCS_URL = "api/IPDocsVerification/GetMedicalCostDocs/"; // URL to fetch documents

  constructor(
    private fb: FormBuilder,
    private dialogRef: MatDialogRef<MedicalCostDocDialogComponent>,
  ...
  ngOnInit(): void {
    ...
    this.setUploadRestrictions(this.data.MedicalCostStauts);
  ...
  openApprovalDialogue(): void {
    ...
    let processKey = EnumApprovalProcess.IPMC;
    const traineeID = this.data.traineeID;

    this.ComSrv.getJSON(`api/IPDocsVerification/GetMedicalCostDocs/${traineeID}`).subscribe({
      next: (response: any) => {
        this.fetchedDocuments = response;

        if (this.fetchedDocuments.length > 0) {
          this.dialogue.openApprovalDialogue(processKey, this.fetchedDocuments[0].MedicalTraineeDocumentsID)
            .subscribe(result => {
              console.log(result);
              // location.reload();
            });
