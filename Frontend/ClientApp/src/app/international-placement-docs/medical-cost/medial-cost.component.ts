...
@Component({
  selector: 'app-medial-cost',
  templateUrl: './medial-cost.component.html',
  styleUrls: ['./medial-cost.component.scss']
})
export class MedicalCostComponent implements OnInit {
  currentUser: any = {}
  schemeArray: any;
  tspDetailArray: any;
  classesArray: any;
    ...
    if (tableData.length > 0) {
      const processedData = tableData.map(row => {
        ...
        if ('IsIPMCDocUploaded' in newRow) {
          ...
          delete newRow['IsIPMCDocUploaded'];
        }
        return newRow;
      });
      this.TableColumns = [
  ...
  openVisaStampingDialog(row: any): void {
    const dialogRef = this.dialog.open(MedicalCostDocDialogComponent, {
      ...
      data: {
        ...
        MedicalCostStauts: row.MedicalCostApprovalStatus
      }
    });
  ...
  openApprovalDialogue(row: any): void {
    ...
    this.ComSrv.getJSON(`api/IPDocsVerification/GetMedicalCostDocs/${row.TraineeID}`).subscribe({
      next: (response: any) => {
        this.fetchedDocuments = response;
