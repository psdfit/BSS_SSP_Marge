...
@Component({
  selector: "app-takamol-cost-doc-dialog",
  templateUrl: "./takamol-cost-doc-dialog.component.html",
  styleUrls: ["./takamol-cost-doc-dialog.component.scss"],
})
export class TakamolCostDocDialogComponent implements OnInit {
  currentUser: any = {}
  ...
  private readonly UPLOAD_URL = "api/IPDocsVerification/UploadTakamolCostDocs"; // Replace with actual URL
  private readonly FETCH_DOCS_URL = "api/IPDocsVerification/GetTakamolCostDocs/"; // URL to fetch documents

  constructor(
    private fb: FormBuilder,
    private dialogRef: MatDialogRef<TakamolCostDocDialogComponent>,
  ...
  openApprovalDialogue(): void {
    ...
    let processKey = EnumApprovalProcess.IPT;
    const traineeID = this.data.traineeID;

    this.ComSrv.getJSON(`api/IPDocsVerification/GetTakamolCostDocs/${traineeID}`).subscribe({
      next: (response: any) => {
        this.fetchedDocuments = response;

        if (this.fetchedDocuments.length > 0) {
          this.dialogue.openApprovalDialogue(processKey, this.fetchedDocuments[0].TakamolDocumentsID)
            .subscribe(result => {
              console.log(result);
              // location.reload();
            });
