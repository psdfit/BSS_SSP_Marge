...
@Component({
  selector: "app-other-training-cost-doc-dialog",
  templateUrl: "./other-training-cost-doc-dialog.component.html",
  styleUrls: ["./other-training-cost-doc-dialog.component.scss"],
})
export class OtherTrainingCostDocDialogComponent implements OnInit {
  currentUser: any = {}
  ...
  private readonly UPLOAD_URL = "api/IPDocsVerification/UploadOtherTrainingCostDocs"; // Replace with actual URL
  private readonly FETCH_DOCS_URL = "api/IPDocsVerification/GetOtherTrainingCostDocs/"; // URL to fetch documents

  constructor(
    private fb: FormBuilder,
    private dialogRef: MatDialogRef<OtherTrainingCostDocDialogComponent>,
  ...
  ngOnInit(): void {
    ...
    this.setUploadRestrictions(this.data.OtherTrainingCostStauts);
  ...
  openApprovalDialogue(): void {
    ...
    let processKey = EnumApprovalProcess.IPOT;
    const traineeID = this.data.traineeID;

    this.ComSrv.getJSON(`api/IPDocsVerification/GetOtherTrainingCostDocs/${traineeID}`).subscribe({
      next: (response: any) => {
        this.fetchedDocuments = response;

        if (this.fetchedDocuments.length > 0) {
          this.dialogue.openApprovalDialogue(processKey, this.fetchedDocuments[0].OtherTraineeDocumentsID)
            .subscribe(result => {
              console.log(result);
              // location.reload();
            });
