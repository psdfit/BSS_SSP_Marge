...
@Component({
  selector: 'app-prometric-cost',
  templateUrl: './prometric-cost.component.html',
  styleUrls: ['./prometric-cost.component.scss']
})
export class PrometricCostComponent implements OnInit {
  currentUser: any = {}
  schemeArray: any;
  tspDetailArray: any;
  classesArray: any;
  ...
  LoadMatTable(tableData: any[]) {
    ...
    if (tableData.length > 0) {
      const processedData = tableData.map(row => {
        ...
        if ('IsIPPCDocUploaded' in newRow) {
          newRow['DocumentsUploaded'] = newRow['IsIPPCDocUploaded'] === 1 ? 'Yes' : 'No';
          delete newRow['IsIPPCDocUploaded'];
        }
        return newRow;
      });
      this.TableColumns = [
  ...
  openVisaStampingDialog(row: any): void {
    const dialogRef = this.dialog.open(PrometricCostDocDialogComponent, {
      width: '1100px',
      data: {
        ...
        PrometricCostStauts: row.PrometricCostApprovalStauts
      }
    });
  ...
  openApprovalDialogue(row: any): void {
    ...
    this.ComSrv.getJSON(`api/IPDocsVerification/GetPrometricCostDocs/${row.TraineeID}`).subscribe({
      next: (response: any) => {
        this.fetchedDocuments = response;
