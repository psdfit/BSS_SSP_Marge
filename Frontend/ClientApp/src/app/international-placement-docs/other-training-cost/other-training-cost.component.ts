...
@Component({
  selector: 'app-other-training-cost',
  templateUrl: './other-training-cost.component.html',
  styleUrls: ['./other-training-cost.component.scss']
})
export class OtherTrainingCostComponent implements OnInit {
  currentUser: any = {}
  schemeArray: any;
  tspDetailArray: any;
  classesArray: any;
  ...
  LoadMatTable(tableData: any[]) {
    ...
    if (tableData.length > 0) {
      const processedData = tableData.map(row => {
        ...
        if ('IsIPOTDocUploaded' in newRow) {
          newRow['DocumentsUploaded'] = newRow['IsIPOTDocUploaded'] === 1 ? 'Yes' : 'No';
          delete newRow['IsIPOTDocUploaded'];
        }
        return newRow;
      });
      this.TableColumns = [
  ...
  openVisaStampingDialog(row: any): void {
    const dialogRef = this.dialog.open(OtherTrainingCostDocDialogComponent, {
      width: '1100px',
      data: {
        ...
        OtherTrainingCostStauts: row.OtherTraineeCostApprovalStatus
      }
    });
  ...
  openApprovalDialogue(row: any): void {
    ...
    this.ComSrv.getJSON(`api/IPDocsVerification/GetOtherTrainingCostDocs/${row.TraineeID}`).subscribe({
      next: (response: any) => {
        this.fetchedDocuments = response;
