...
@Component({
  selector: 'app-takamol-cost',
  templateUrl: './takamol-cost.component.html',
  styleUrls: ['./takamol-cost.component.scss']
})
export class TakamolCostComponent implements OnInit {
  currentUser: any = {}
  schemeArray: any;
  tspDetailArray: any;
  classesArray: any;
  ...
  LoadMatTable(tableData: any[]) {
    ...
    if (tableData.length > 0) {
      const processedData = tableData.map(row => {
        ...
        if ('IsIPTDocUploaded' in newRow) {
          newRow['DocumentsUploaded'] = newRow['IsIPTDocUploaded'] === 1 ? 'Yes' : 'No';
          delete newRow['IsIPTDocUploaded'];
        }
        return newRow;
      });
      this.TableColumns = [
  ...
  openVisaStampingDialog(row: any): void {
    ...
    const dialogRef = this.dialog.open(TakamolCostDocDialogComponent, {
      width: '1100px',
      data: {
        ...
        TakamolCostStauts: row.TakamolCostApprovalStatus
      }
    });
  ...
  openApprovalDialogue(row: any): void {
    ...
    this.ComSrv.getJSON(`api/IPDocsVerification/GetTakamolCostDocs/${row.TraineeID}`).subscribe({
      next: (response: any) => {
        this.fetchedDocuments = response;
