...
@Component({
  selector: 'app-ojt-deo-verification',
  templateUrl: './ojt-deo-verification.component.html',
  styleUrls: ['./ojt-deo-verification.component.scss'],
  animations: [enterAnimation],
  providers: [GroupByPipe, DatePipe]
})
export class OjtDeoVerificationComponent implements OnInit {
  ...
  getDeoDashboardStats() {
    this.comSrv.getJSON(`api/TSPEmployment/GetDeoDashboardStatsOJT`).subscribe(
      (d: any) => {
        console.log(d);
  ...
  getVerificationDropdowns() {
    this.comSrv.getJSON(`api/TSPEmployment/EmployeeVerificationDropdownOJT`).subscribe(
      (d: any) => {
        console.log(d);
  ...
  GetTSPs() {
    this.comSrv.getJSON(`api/TSPEmployment/GetTspForEmploymentVerificationOJT?placementTypeId=${this.filters.PlacementTypeID}&verificationMethodId=${this.filters.VerificationMethodID}`).subscribe(
      (d: any) => {
  ...
  GetEmploymentClassesByTSP() {
    this.comSrv.getJSON(`api/TSPEmployment/GetEmploymentVerificationClassesOJT?pId=${this.filters.PlacementTypeID}&vmId=${this.filters.VerificationMethodID}&tspId=${this.filters.TSPID}`).subscribe(
      (d: any) => {
        if (this.filters.ClassID == 0) {
  ...
  GetClasses() {
    this.comSrv.getJSON(`api/TSPEmployment/GetTspClassesOJT`).subscribe(
      (d: any) => {
        this.ClassList = d.ClassList;
        this.ClassListFiltered = d.ClassList;
        console.log(d);
  ...
  GetPSPTrainees() {
    this.comSrv.postJSON(`api/PSPEmployment/GetPSPTraineeForDEOVerificationOJT`, this.filtersPSP).subscribe(
      (d: any) => {
  ...
  GetPSPBatchesList() {
    this.comSrv.getJSON(`api/PSPEmployment/GetPSPBatchesOJT`).subscribe(
      (d: any) => {
        this.PSPBatches = d[0];
  ...
  toggleVerify(Row: any) {
    this.comSrv.postJSON("api/EmploymentVerification/UpdateVerifyStatusOJT/", { PlacementID: Row.PlacementID, IsVerified: true }).subscribe(
      (d: any) => {
  ...
  GetSelfEmploymentList() {
    ...
    this.comSrv.getJSON(`api/TSPEmployment/GetSelfEmploymentListOJT?pId=${this.filters.PlacementTypeID}&vmId=${this.filters.VerificationMethodID}&cId=${this.filters.ClassID}`).subscribe(
      (d: any) => {
  ...
  GetPSPSelfEmploymentList() {
    ...
    this.comSrv.getJSON(`api/PSPEmployment/GetPSPSelfEmploymentListOJT?pId=${this.filtersPSP.PlacementTypeID}&vmId=${this.filtersPSP.VerificationMethodID}&bId=${this.filtersPSP.PSPBatchID}`).subscribe(
      (d: any) => {
  ...
  openDialog(item: any): void {
    this.comSrv.getJSON(`api/TSPEmployment/GetTraineeForEmploymentVerificationOJT?traineeId=${item.TraineeID}`).subscribe(
      (d: any) => {
  ...
  pushDataToDialog() {
    const dialogRef = this.dialog.open(EmpVerificationOJTComponent, {
      minWidth: '1000px',
      minHeight: '600px',
  ...
  getTSPemploymentList(item: any, TSPID: any) {
    ...
    this.comSrv.getJSON(`api/TSPEmployment/GetSelfEmploymentListOJT?pId=${this.filters.PlacementTypeID}&vmId=${this.filters.VerificationMethodID}&cId=${item}&TSPID=${TSPID}`).subscribe(
      (d: any) => {
  ...
  getPSPemploymentList(item: any) {
    ...
    //this.specificTraineeListForVerification = this.allTraineeListForVerification.filter(i => i.ClassID == item.value);
    this.comSrv.getJSON(`api/PSPEmployment/GetPSPSelfEmploymentListOJT?pId=${this.filtersPSP.PlacementTypeID}&vmId=${this.filtersPSP.VerificationMethodID}&bId=${item}`).subscribe(
      (d: any) => {
        this.specificTraineeListForVerification = d.SelfList;
        this.EmploymentTypeID = d.TypeID;
        if (this.specificTraineeListForVerification.length == 0) {
  ...
  exportToExcelTraineesData() {
    this.comSrv.getJSON(`api/TSPEmployment/TraineeForVerificationExportExcelOJT?pId=${this.filters.PlacementTypeID}&vmId=${this.filters.VerificationMethodID}&tspId=${this.filters.TSPID}&cId=${this.filters.ClassID}`).subscribe(
      (d: any) => {
  ...
  openVerificationDialog(item: any): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.GetSelfEmploymentList();
        // Collapse all opened table rows by resetting HasTrainees
        this.ClassListFiltered.forEach(row => {
          row.HasTrainees = false;
        });
  ...
  GetTraineeOfClass(ClassID: any, r: any) {
    ...
    this.comSrv.getJSON(`api/TSPEmployment/GetTraineesForEmploymentVerificationOJT?pId=${this.filters.PlacementTypeID}&vmId=${this.filters.VerificationMethodID}&cId=${ClassID}`).subscribe(
      (d: any) => {
        debugger;
  ...
  SubmitClassData(ClassID: any, ClassCode: string) {
    ...
    this.comSrv.confirm('Class Employment verification submit Confirmation', Dif > 0 ? Dif + ' Trainee data not saved Are you sure to submit?' : 'Are you sure to submit?').subscribe((res) => {
      if (res == true) {
        this.comSrv.postJSON("api/EmploymentVerification/SubmitClassVerificationOJT", { 'ClassID': ClassID }).subscribe((d: any) => {
  ...
  forwardToTelephonic() {
    ...
    this.comSrv.confirm("Are you sure?", "You want to forward to Telephonic verification").subscribe(result => {
      ...
      if (!list || list.length <= 0) {
        this.comSrv.ShowError("There is nothing to move");
        return;
      }
      var ids = list.map(x => x.ClassID);
      this.filters.PlacementTypeID;
      this.filters.VerificationMethodID;
      this.comSrv.postJSON('api/TSPEmployment/ForwardedToTelephonicOJT', { ClassIDslist: ids, PlacementTypeID: this.filters.PlacementTypeID, VerificationMethodID: this.filters.VerificationMethodID })
        .subscribe((d: any) => {
          if (d) {
