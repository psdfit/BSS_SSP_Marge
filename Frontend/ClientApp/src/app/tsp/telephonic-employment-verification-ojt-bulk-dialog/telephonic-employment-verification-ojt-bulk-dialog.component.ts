...
//};
@Component({
  selector: 'app-telephonic-employment-verification-ojt-bulk-dialog',
  templateUrl: './telephonic-employment-verification-ojt-bulk-dialog.component.html',
  styleUrls: ['./telephonic-employment-verification-ojt-bulk-dialog.component.scss']
})
export class TelephonicEmploymentVerificationOJTBulkComponent implements OnInit {
  ExcelFileForm: any;
  ...
  constructor(private fb: FormBuilder,
    private el: ElementRef, private ComSrv: CommonSrvService, private dialogRef: MatDialogRef<TelephonicEmploymentVerificationOJTBulkComponent>, public domSanitizer: DomSanitizer,
  ...
  GenerateForm(itm: any) {
    let form: FormGroup = this.fb.group({
      ...
      IsVerified: [itm.IsVerified],
      Comments: [itm.Comments],
      CallCenterVerificationTraineeID: [itm.CallCenterVerificationTraineeID, Validators.required],
      CallCenterVerificationSupervisorID: [itm.CallCenterVerificationSupervisorID, Validators.required],
      CallCenterVerificationCommentsID: [itm.CallCenterVerificationCommentsID, Validators.required],
    }, { updateOn: "blur" });
  ...
  Submit(nform: FormGroupDirective) {
    ...
    if(this.Detail.value.some(person => person.CallCenterVerificationTraineeID === 0 || person.CallCenterVerificationSupervisorID===0 || person.CallCenterVerificationCommentsID===0)){
      this.ComSrv.ShowWarning("Required All Fields of Call Center Verification");
      return;
      } 
    this.ComSrv.postJSON('api/EmploymentVerification/TelephonicSaveBulkOJT', this.Detail.getRawValue())// { 'json': JSON.stringify(dataObj) })
      .subscribe((d: any) => {
        nform.resetForm();
  ...
  getData() {
    if (this.data[0].ClassID) {
      this.ComSrv.postJSON('api/TSPEmployment/GetEmploymentDataForVerificationOJT', { "ClassID": this.data[0].ClassID }).subscribe((d: any) => {
  ...
  get CallCenterVerificationTraineeID() { return this.traineeEmploymentForm.get("CallCenterVerificationTraineeID"); }
  get CallCenterVerificationSupervisorID() { return this.traineeEmploymentForm.get("CallCenterVerificationSupervisorID"); }
  get CallCenterVerificationCommentsID() { return this.traineeEmploymentForm.get("CallCenterVerificationCommentsID"); }
