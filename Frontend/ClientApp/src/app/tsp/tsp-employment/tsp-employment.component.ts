  ...
  if (fg.get("EmploymentStatus").value == "Employed") {
    if (PlacementTypeID == 1 && VerificationMethodId != 9) {
      ...
      if (PlacementTypeID == 2 && VerificationMethodId == 6) {
        return fg.get("FilePath").value != "" || fg.get("EOBI").value != ""
          ? null
          : { fileoeobi: true };
      } else return fg.get("FilePath").value != "" ? null : { file: true };
    }
  } else fg.get("FilePath").clearValidators();
};
@Component({
  selector: "hrapp-tsp",
  templateUrl: "./tsp-employment.component.html",
  styleUrls: ["./tsp-employment.component.scss"],
  animations: [
...
export class TSPEmpComponent implements OnInit {
  ...
  displayedColumns = [
    ...
    //'SupervisorName',
    "Action",
  ]; //, 'SupervisorContact', 'EmploymentStartDate', 'EmploymentStatus', 'EmploymentType', 'EmployerName', 'EmployerBusinessType', 'EmploymentAddress', 'District', 'EmploymentTehsil', 'EmploymentTiming', 'IsTSP', 'OldID', 'EmploymentTehsilOld', 'OfficeContactNo', 'IsMigrated', 'TraineeName', "Action"];
  ClassDrp: any;
  ...
  EnText: string = " Trainee Employment Form";
  title: string;
  savebtn: string = "Save";
  working: boolean;
  error: string;
  ...
  constructor(
    ...
    this.EmploymentStatus.valueChanges.subscribe((v) => {
      ...
      for (const field in this.traineeEmploymentForm.controls) {
        // 'field' is a string
  ...
  jsDataObject: any;
  getData() {
    this.ComSrv.postJSON("api/TSPEmployment/GetDataForEmployment", {
      ClassID: this.paramClassId,
      TraineeID: this.paramTraineeId,
    }).subscribe(
      (d: any) => {
        console.log(d);
        this.jsDataObject = d;
        ...
        if (d.DeadlineStatus == "Date Passed") {
          this.ComSrv.ShowError(
            "Can't submit the request. Deadline end",
            "Error"
          );
        ...
        if (this.trineeProfile) {
          this.traineeEmploymentForm.patchValue({
            ...
            TimeFrom: this.trineeProfile.EmploymentTiming
              ? this.trineeProfile.EmploymentTiming.split(" to ")[0]
              : "",
            TimeTo: this.trineeProfile.EmploymentTiming
              ? this.trineeProfile.EmploymentTiming.split(" to ")[1]
              : "",
            //EmploymentTiming: ['', [Validators.required]],
          ...
          if (this.trineeProfile.PlacementTypeID) {
            this.VerificationMethodsDrp = this.VerificationMethods.filter(
              (x) => x.PlacementTypeID == this.trineeProfile.PlacementTypeID
            );
          }
        }
      },
  ...
  getTraineesEmploymentFile(row) {
    ...
    this.ComSrv.postJSON("api/TSPEmployment/GetEmploymentDataByTraineeID", {
      ClassID: classId,
      TraineeID: traineeId,
    }).subscribe(
      (d: any) => {
  ...
  GetEmploymentType() {
    ...
    if (employmentStatus == "Employed") {
      ...
      if (this.ClassDrp.EmploymentCommitmentFormal == 0) {
        this.PlacementTypeDrp = this.jsDataObject.PlacementTypes.filter(
          (x) => x.PlacementType == "self"
        );
      } else if (this.ClassDrp.EmploymentCommitmentSelf == 0) {
        this.PlacementTypeDrp = this.jsDataObject.PlacementTypes.filter(
          (x) => x.PlacementType == "Formal"
        );
      } else {
        this.PlacementTypeDrp = this.jsDataObject.PlacementTypes;
      }
    } else {
      this.PlacementTypeDrp = this.jsDataObject.PlacementTypes;
    }
  ...
  // employmentCheck(employmentType: string = "self",IsEdit:number=0) {
  //   debugger;

  //   const employmentCount = this.EmploymentData.filter(
  //     (x) => x.EmploymentStatus == "Employed"
  //   ).length;

  //   if(IsEdit==0){
  //     if (this.EmploymentStatus.value == "Employed") {
  //       this.working = false;
  //       if (employmentType == "self") {
  //         if (
  //           employmentCount <
  //           this.jsDataObject.ClassDetail[0].SelfEmploymentCommitmentCount
  //         ) {
  //           this.working = false; 
  //         } else {
  //           this.ComSrv.ShowError("employment limit has been completed");
  //           this.working = true;
  //         }
  //       } else {
  //         if (employmentCount <  this.jsDataObject.ClassDetail[0].FormalEmploymentCommitmentCount) {
  //           this.working = false; 
  //         } else {
  //           this.ComSrv.ShowError("employment limit has been completed");
  //           this.working = true;
  ...
  employmentCheck(employmentType: string = "self", IsEdit: number = 0) {
    ...
    // Helper function to handle employment validation logic

    const validateEmploymentLimit = (commitmentCount: number) => {
      if (employmentCount < commitmentCount) {
        this.working = false;
      } else {
        this.ComSrv.ShowError("Employment limit has been completed");
        this.working = true;
    ...
    // Main logic for checking employment status
    if ((IsEdit === 0 && this.EmploymentStatus.value === "Employed") ||   (IsEdit === 1 && this.EmploymentStatus.value !== "Employed")) {
      this.working = false;
      // Validate based on employment type (self or formal)
      const commitmentCount = employmentType === "self"? this.jsDataObject.ClassDetail[0].SelfEmploymentCommitmentCount : this.jsDataObject.ClassDetail[0].FormalEmploymentCommitmentCount;
      validateEmploymentLimit(commitmentCount);
    }
  }
  ...
  toggleActive(row) {
    this.ComSrv.confirm().subscribe((result) => {
      if (result) {
        this.ComSrv.postJSON("api/PlacementFormE/ActiveInActive", {
          ...
          (d: any) => {
            this.ComSrv.openSnackBar(
              row.InActive == true
                ? environment.InActiveMSG.replace("${Name}", this.EnText)
                : environment.ActiveMSG.replace("${Name}", this.EnText)
            );
            // this.placementforme =new MatTableDataSource(d);
          },
          (error) => this.ComSrv.ShowError(this.error, this.EnText)
        ); // error path
      } else {
  ...
  get PlacementID() {
    return this.traineeEmploymentForm.get("PlacementID");
  }
  get TraineeID() {
    return this.traineeEmploymentForm.get("TraineeID");
  }
  get ClassID() {
    return this.traineeEmploymentForm.get("ClassID");
  }
  get Designation() {
    return this.traineeEmploymentForm.get("Designation");
  }
  get PlatformName() {
    return this.traineeEmploymentForm.get("PlatformName");
  }
  get NameofTraineeStorePage() {
    return this.traineeEmploymentForm.get("NameofTraineeStorePage");
  }
  get LinkofTraineeStorePage() {
    return this.traineeEmploymentForm.get("LinkofTraineeStorePage");
  }
  get Department() {
    return this.traineeEmploymentForm.get("Department");
  }
  get EmploymentDuration() {
    return this.traineeEmploymentForm.get("EmploymentDuration");
  }
  get Salary() {
    return this.traineeEmploymentForm.get("Salary");
  }
  get SupervisorName() {
    return this.traineeEmploymentForm.get("SupervisorName");
  }
  get SupervisorContact() {
    return this.traineeEmploymentForm.get("SupervisorContact");
  }
  get EmploymentStartDate() {
    return this.traineeEmploymentForm.get("EmploymentStartDate");
  }
  get EmploymentStatus() {
    return this.traineeEmploymentForm.get("EmploymentStatus");
  }
  get EmploymentType() {
    return this.traineeEmploymentForm.get("EmploymentType");
  }
  get EmployerName() {
    return this.traineeEmploymentForm.get("EmployerName");
  }
  get EmployerNTN() {
    return this.traineeEmploymentForm.get("EmployerNTN");
  }
  get EmployerBusinessType() {
    return this.traineeEmploymentForm.get("EmployerBusinessType");
  }
  get EmploymentAddress() {
    return this.traineeEmploymentForm.get("EmploymentAddress");
  }
  get District() {
    return this.traineeEmploymentForm.get("District");
  }
  get EmploymentTehsil() {
    return this.traineeEmploymentForm.get("EmploymentTehsil");
  }
  get TimeFrom() {
    return this.traineeEmploymentForm.get("TimeFrom");
  }
  get TimeTo() {
    return this.traineeEmploymentForm.get("TimeTo");
  }
  get EmploymentTiming() {
    return this.traineeEmploymentForm.get("EmploymentTiming");
  }
  get IsTSP() {
    return this.traineeEmploymentForm.get("IsTSP");
  }
  get OldID() {
    return this.traineeEmploymentForm.get("OldID");
  }
  get EmploymentTehsilOld() {
    return this.traineeEmploymentForm.get("EmploymentTehsilOld");
  }
  get OfficeContactNo() {
    return this.traineeEmploymentForm.get("OfficeContactNo");
  }
  get IsMigrated() {
    return this.traineeEmploymentForm.get("IsMigrated");
  }
  get TraineeName() {
    return this.traineeEmploymentForm.get("TraineeName");
  }
  get ContactNumber() {
    return this.traineeEmploymentForm.get("ContactNumber");
  }
  get TraineeCode() {
    return this.traineeEmploymentForm.get("TraineeCode");
  }
  get ClassCode() {
    return this.traineeEmploymentForm.get("ClassCode");
  }
  get VerificationMethodId() {
    return this.traineeEmploymentForm.get("VerificationMethodId");
  }
  get PlacementTypeID() {
    return this.traineeEmploymentForm.get("PlacementTypeID");
  }
  get FileType() {
    return this.traineeEmploymentForm.get("FileType");
  }
  get FileName() {
    return this.traineeEmploymentForm.get("FileName");
  }
  get FilePath() {
    return this.traineeEmploymentForm.get("FilePath");
  }
  ...
  radioChange(event) {
    ...
    this.VerificationMethodsDrp = this.VerificationMethods.filter(
      (x) => x.PlacementTypeID == event.value
    );
    if (event.value == 1) {
      debugger;
      this.Designation.disable();
  ...
  Submit(myform: FormGroupDirective) {
    if (
      this.PlacementTypeID.value == 1 &&
      this.VerificationMethodId.value != 9
    ) {
      this.PlatformName.disable();
      this.NameofTraineeStorePage.disable();
      this.LinkofTraineeStorePage.disable();
    ...
    this.ComSrv.postFile("api/TSPEmployment/Save", formValues).subscribe(
      (result: any) => {
        if (result) {
          this.getData();
  ...
  setFileType() {
    if (this.EmploymentStatus.value == EmploymentStatuses.Employed) {
      if (
        this.PlacementTypeID.value == "2" &&
        this.VerificationMethodId.value == "6"
      ) {
        this.FileType.setValue("Employment Letter | Salary Slip");
      } else if (this.VerificationMethodId.value == "9") {
        this.FileType.setValue("Self");
      }
  ...
  getFormValidationErrors() {
    Object.keys(this.traineeEmploymentForm.controls).forEach((key) => {
      const controlErrors: ValidationErrors =
        this.traineeEmploymentForm.get(key).errors;
