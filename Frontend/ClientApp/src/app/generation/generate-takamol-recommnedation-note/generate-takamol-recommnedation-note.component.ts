...
@Component({
  selector: "app-generate-takamol-recommnedation-note.component",
  templateUrl: "./generate-takamol-recommnedation-note.component.html",
  styleUrls: ["./generate-takamol-recommnedation-note.component.scss"],
  providers: [
    {
      provide: DateAdapter,
...
export class GenerateTakamolRecommnedationNoteComponent implements OnInit {
  Scheme = [];
  Kam = [];
  TspDetail = [];
  PrnClasses: any = [];
  ...
  constructor(
    ...
    this.http.setTitle("Generate Takamol Recommnedation Note");
  ...
  getTspClassesForVRNCompletion(value) {
    ...
    this.IsGenerated = false;
    this.http
      .postJSON(`api/ManualNoteGeneration/GetTakamolRecommnedationNote`, this.filters)
      .subscribe(
        (data: any) => {
          this.error = "";
          debugger;
  ...
  generate() {
    this.delay(1000);
    this.http
      .confirm("Takamol Recommendation Note Generation", "Are You Sure you want to generate Takamol Recommendation Note?")
      .subscribe((result) => {
        if (result) {
          this.http
            .postJSON(`api/ManualNoteGeneration/GenerateTakamolRecommnedationNote`, this.filters)
            .subscribe(
              (data: any) => {
                if (data) {
                  this.PrnClasses = [];
  ...
  exportToExcel() {
    ...
    const exportExcel: ExportExcel = {
      Title: "Takamol Recommnedation Note Report",
