...
@Component({
  selector: 'app-generate-ojt',
  templateUrl: './generate-ojt.component.html',
  styleUrls: ['./generate-ojt.component.scss'],
  providers: [
...
export class GenerateOjtComponent implements OnInit {
  ...
  displayedColumnsClass = ['Sr', 'SchemeName', 'TradeName', 'TSPName', 'ClassCode', 'TraineesPerClass', 'ReportedTrainees', 'EmployedTrainees', 'OJTVerified','OJTUnverified'];
  ...
  constructor(private http: CommonSrvService, private fb: FormBuilder, private dialogue: DialogueService) {
    this.http.setTitle('Generate OJT');
  }
  ...
  getSchemes() {
    this.http.getJSON(`api/Scheme/GetSchemeByIDOJT`).subscribe(
      (d: any) => {
        this.error = '';
        this.Scheme = d;
      },
  ...
  getTspClassesForPRNCompletion(value) {
    ...
    this.http.postJSON(`api/SRNCoursera/GetOJTClasses`, this.filters).subscribe(
      (data: any) => {
        this.error = '';
  ...
  generatePrnCompletion() {
    ...
    this.http.confirm('OJT Generation', 'Are You Sure you want to generate OJT?').subscribe(result => {
      if (result) {
        this.http.postJSON(`api/SRNCoursera/GenerateSRNOJT`, this.filters).subscribe(
          (data: any) => {
            if (data) {
              this.PrnClasses = [];
  ...
  chosenMonthHandler(normalizedMonth: Moment, datepicker: MatDatepicker<Moment>) {
    ...
    if (this.schemeFilter.value !== 0)
      this.getTspClassesForPRNCompletion(this.schemeFilter.value);
    datepicker.close();
  }
