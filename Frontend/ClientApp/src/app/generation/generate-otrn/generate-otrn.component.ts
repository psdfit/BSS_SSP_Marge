...
@Component({
  selector: "app-generate-otrn",
  templateUrl: "./generate-otrn.component.html",
  styleUrls: ["./generate-otrn.component.scss"],
  providers: [
    {
      provide: DateAdapter,
...
export class GenerateOtrnComponent implements OnInit {
  Scheme = [];
  Kam = [];
  TspDetail = [];
  PrnClasses: any = [];
  ...
  constructor(
    ...
    this.http.setTitle("Generate OTRN");
  ...
  getTspClassesForVRNCompletion(value) {
    ...
    this.IsGenerated = false;
    this.http
      .postJSON(`api/ManualNoteGeneration/GetOTRN`, this.filters)
      .subscribe(
        (data: any) => {
          this.error = "";
          debugger;
  ...
  generate() {
    this.delay(1000);
    this.http
      .confirm("OTRN Generation", "Are You Sure you want to generate OTRN?")
      .subscribe((result) => {
        if (result) {
          this.http
            .postJSON(`api/ManualNoteGeneration/GenerateOTRN`, this.filters)
            .subscribe(
              (data: any) => {
                if (data) {
                  this.PrnClasses = [];
