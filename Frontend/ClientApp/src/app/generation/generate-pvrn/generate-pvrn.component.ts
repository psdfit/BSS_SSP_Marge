...
@Component({
  selector: "app-generate-pvrn",
  templateUrl: "./generate-pvrn.component.html",
  styleUrls: ["./generate-pvrn.component.scss"],
  providers: [
    {
      provide: DateAdapter,
...
export class GeneratePvrnComponent implements OnInit {
  Scheme = [];
  Kam = [];
  TspDetail = [];
  PrnClasses: any = [];
  ...
  constructor(
    ...
    this.http.setTitle("Generate PVRN");
  ...
  getTspClassesForVRNCompletion(value) {
    ...
    this.IsGenerated = false;
    this.http
      .postJSON(`api/ManualNoteGeneration/GetPVRN`, this.filters)
      .subscribe(
        (data: any) => {
          this.error = "";
          debugger;
  ...
  generate() {
    this.delay(1000);
    this.http
      .confirm("PVRN Generation", "Are You Sure you want to generate PVRN?")
      .subscribe((result) => {
        if (result) {
          this.http
            .postJSON(`api/ManualNoteGeneration/GeneratePVRN`, this.filters)
            .subscribe(
              (data: any) => {
                if (data) {
                  this.PrnClasses = [];
