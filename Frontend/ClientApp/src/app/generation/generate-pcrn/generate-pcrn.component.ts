...
@Component({
  selector: "app-generate-pcrn",
  templateUrl: "./generate-pcrn.component.html",
  styleUrls: ["./generate-pcrn.component.scss"],
  providers: [
    {
      provide: DateAdapter,
...
export class GeneratePcrnComponent implements OnInit {
  Scheme = [];
  Kam = [];
  TspDetail = [];
  PrnClasses: any = [];
  ...
  constructor(
    ...
    this.http.setTitle("Generate PCRN");
  ...
  getTspClassesForVRNCompletion(value) {
    ...
    this.IsGenerated = false;
    this.http
      .postJSON(`api/ManualNoteGeneration/GetPCRN`, this.filters)
      .subscribe(
        (data: any) => {
          this.error = "";
          debugger;
  ...
  generate() {
    this.delay(1000);
    this.http
      .confirm("PCRN Generation", "Are You Sure you want to generate PCRN?")
      .subscribe((result) => {
        if (result) {
          this.http
            .postJSON(`api/ManualNoteGeneration/GeneratePCRN`, this.filters)
            .subscribe(
              (data: any) => {
                if (data) {
                  this.PrnClasses = [];
