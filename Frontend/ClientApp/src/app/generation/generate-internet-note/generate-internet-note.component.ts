...
@Component({
  selector: 'app-generate-internet-note',
  templateUrl: './generate-internet-note.component.html',
  styleUrls: ['./generate-internet-note.component.scss'],
...
export class GenerateInternetNoteComponent implements OnInit {
  Scheme = [];
  Kam = [];
  TspDetail = [];
  PrnClasses: any = [];
  ...
  constructor(
    ...
    this.http.setTitle("Generate InetrnetNote");
  ...
  getSchemes() {
    this.http.getJSON("api/Scheme/FetchSchemeForFilter").subscribe(
      (d: any) => {
        this.error = "";
        this.Scheme = d.filter((x) => this._date.transform(x.CreatedDate, "yyyy") > '2025');
      },
      (error) => {
        this.error = error;
      }
  ...
  getTspClassesForVRNCompletion(value) {
    ...
    this.http
      .postJSON(`api/ManualNoteGeneration/GetInternetCostReport`, this.filters)
      .subscribe(
        (data: any) => {
          this.error = "";
          debugger;
  ...
  generate() {
    this.delay(1000);
    this.http
      .confirm("Internet / Dongal Generation", "Are You Sure you want to generate Internet Note?")
      .subscribe((result) => {
        if (result) {
          this.http
            .postJSON(`api/ManualNoteGeneration/GenerateInternetNote`, this.filters)
            .subscribe(
              (data: any) => {
                if (data) {
                  this.PrnClasses = [];
  ...
  exportToExcel() {
    ...
    const exportExcel: ExportExcel = {
      Title: "Internet-Dongal Generation Report",
