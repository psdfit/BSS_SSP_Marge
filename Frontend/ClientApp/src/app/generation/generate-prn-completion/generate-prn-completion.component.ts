...
@Component({
  selector: 'app-generate-prn-completion',
  templateUrl: './generate-prn-completion.component.html',
  styleUrls: ['./generate-prn-completion.component.scss'],
  providers: [
...
export class GeneratePrnCompletionComponent implements OnInit, AfterViewInit {
  ...
  getTspClassesForPRNCompletion(value) {
    ...
    this.isValid = false;
    this.IsGenerated = false;
    this.http.postJSON(`api/Generation/RD_ClassesForPRNCompletion`, this.filters) .subscribe(
      (data: any) => {
        this.error = '';
