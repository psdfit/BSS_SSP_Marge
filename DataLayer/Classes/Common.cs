...
namespace DataLayer.Classes
{
    public static class Common
    {
        ...
        public static List<EmploymentStatusModel> FilterPlacementDropdown(List<EmploymentStatusModel> dropdown)
        {
            dropdown = dropdown.Where(m => m.EmploymentStatusName != "Student" && m.EmploymentStatusName != "Unemployed").ToList();
            return dropdown;
        }
        ...
        public static string GetFileBase64(string path)
        {
            ...
            if (AppContext.BaseDirectory.IndexOf("bin") > 0)
                ...
                appRoot = AppContext.BaseDirectory;
            if (File.Exists(appRoot + @path))
            {
                byte[] bytes = System.IO.File.ReadAllBytes(appRoot + @path);
                base64 = Convert.ToBase64String(bytes);
                base64 = "data:" + MimeTypesMap.GetMimeType(path) + ";Base64," + base64;
            }
            return base64;
        }
        ...
        //public static UsersModel GetUserFromRequest(HttpRequest Request)
        //{
        //    if (Request.Headers.ContainsKey("Authorization"))
        //    {
        //        //Request.Headers["Authorization"];
        //        return new SRVUsers().GetByUserID(Convert.ToInt32(ClaimTypes.NameIdentifier.ToString())); ;
        //    }
        //    else
        //        return null;
        //}

        //public static string Export(LocalReport rpt, string fileType = "PDF")
        //{
        //    try
        //    {
        //        byte[] bytes = rpt.Render(fileType, null, out string mimeType, out string encoding, out string extension, out string[] streamids, out Warning[] warnings);
        ...
        public static string AddFile(object filePath, string path)
        {
        ...
        public static string[] ConvertDtToStringArray(DataTable dt, string cloumName)
        {
            List<string> myList = new List<string>();
            foreach (DataRow row in dt.Rows)
            {
                myList.Add(row[cloumName].ToString());
            }
        ...
        {
            if (!IsnullOrEmpty(val)) return new DateTime();
            return Convert.ToDateTime(val);
        }

        public static string GetFieldVal(string Field, string Table, string Where = null)
        {
            DataTable dt = SqlHelper.ExecuteDataset(SqlHelper.GetCon(), CommandType.Text, "SELECT " + Field + " FROM " + Table + ((Where == null) == false ? " WHERE " + Where : "")).Tables[0];
            if (dt.Rows.Count > 0)
            {
        ...
        //public static DataTable ExceltoDataTable(string ExcelFile)
        //{
        //    string conStr = ConfigurationManager.ConnectionStrings["Excel07ConString"].ConnectionString;
        //    conStr = string.Format(conStr, ExcelFile, true);
        ...
        //public static DataTable ExceltoDataTableForImport(string ExcelFile)
        //{
        //    string conStr = ConfigurationManager.ConnectionStrings["Excel07ConString"].ConnectionString;
        //    conStr = string.Format(conStr, ExcelFile, true);
        //    OleDbConnection connExcel = new OleDbConnection(conStr);
        ...
        //    connExcel.Open();
        //    DataTable dtExcelSchema = connExcel.GetOleDbSchemaTable(OleDbSchemaGuid.Tables, null);
        //    _ = dtExcelSchema.Rows[0]["TABLE_NAME"].ToString();
        //    connExcel.Close();
        //    //Read Data from First Sheet
        //    connExcel.Open();
        //    cmdExcel.CommandText = "SELECT * From [" + "Services$" + "]";
        //    oda.SelectCommand = cmdExcel;
        //    oda.Fill(dt);
        //    connExcel.Close();
        //    oda.Dispose();
        ...
        public static SqlParameter[] GetParams(object obj)
        {
            if (obj == null)
                return null;
            IEnumerable<PropertyInfo> props = obj.GetType().GetProperties().Where(pi => IsnullOrEmpty(pi.GetValue(obj)));
            SqlParameter[] param = new SqlParameter[Convert.ToInt32(props.LongCount())];
            Int32 i = 0;
            foreach (var p in props)
            {
                param[i] = new SqlParameter("@" + p.Name, p.GetValue(obj));
                i++;
            }
        ...
        public static List<SqlParameter> GetPagingParams(PagingModel obj)
        {
            List<SqlParameter> param = new List<SqlParameter>();

            param.Add(new SqlParameter("@PageNo", obj.PageNo));
            param.Add(new SqlParameter("@PageSize", obj.PageSize));
            param.Add(new SqlParameter("@SortColumn", obj.SortColumn));
            param.Add(new SqlParameter("@SortOrder", obj.SortOrder));
            if (!string.IsNullOrEmpty(obj.SearchColumn))
                param.Add(new SqlParameter("@" + obj.SearchColumn, obj.SearchValue));
        ...
        public static void SendEmailPushNotification(string logFilePath, string MailTo, string Subject, string Body, bool isWithHeaderFooter = false)
        {
            ...
            //    else
            //        appRoot = AppContext.BaseDirectory;
            //    AlternateView view = AlternateView.CreateAlternateViewFromString(Body, new ContentType("text/html"));
            //    LinkedResource header = new LinkedResource($@"{appRoot}/Assets/Images/header_email.png");
            //    LinkedResource footer = new LinkedResource($@"{appRoot}/Assets/Images/footer_email.jpg");
            //    header.ContentId = "Header";
            //    footer.ContentId = "Footer";
            //    view.LinkedResources.Add(header);
            //    view.LinkedResources.Add(footer);
            ...
            SmtpClient SmtpServer = new SmtpClient(mailServer)
            {
                ...
                EnableSsl = true,
                Timeout = 60000,
                //Credentials = new NetworkCredential("psdftest@gmail.com", "Abc@12345")
        ...
        public static void SendEmail(string MailTo, string Subject, string Body, bool isWithHeaderFooter = false, string EmailAttachmentFil = null)
        {
            IConfigurationSection email = configuration.GetSection("AppSettings").GetSection("Email");
        ...
        //public static async Task SendEmailAsync(string MailTo, string Subject, string Body, CancellationToken cancellationToken, bool isWithHeaderFooter = false, string EmailAttachmentFil = null)
        //{
        //    IConfigurationSection email = configuration.GetSection("AppSettings").GetSection("Email");
        ...
        //    mail.Subject = Subject;
        //    mail.IsBodyHtml = true;
        //    //if (EmailAttachmentFil != null && EmailAttachmentFil != "")
        //    //{
        //    //    string Extention = GetFileExtension(EmailAttachmentFil);
        //    //    string EncodedExtention = GetMimeType(Extention);
        //    //    var imageBytes = Convert.FromBase64String(EmailAttachmentFil);
        //    //    var stream = new MemoryStream(imageBytes);
        //    //    ContentType ct = new ContentType(EncodedExtention);
        ...
        public static async Task SendEmailAsync(string MailTo, string Subject, string Body, CancellationToken cancellationToken, bool isWithHeaderFooter = false, string EmailAttachmentFil = null)
        {
            ...
            while (!isEmailSent && currentAttempt < maxRetryAttempts)
            {
                try
                {
                    IConfigurationSection email = configuration.GetSection("AppSettings").GetSection("Email");
                ...
                catch (Exception ex)
                {
                    ...
                    Console.WriteLine($"Error sending email attempt {currentAttempt}: {ex.Message}");
                    await Task.Delay(1000); // Wait for a short time before retrying
                }
            }

            if (!isEmailSent)
            {
                // Handle the case where email sending failed after multiple attempts
                Console.WriteLine($"Failed to send email to {MailTo} after {maxRetryAttempts} attempts.");
            }
        }


        public static void sendEmailtoUser(string MailTo, string Subject, string Body, bool isWithHeaderFooter = false, string EmailAttachmentFil = null)
        {
            ...
            MailMessage mail = new MailMessage(mailAddress, MailTo, Subject, Body);
        ...
        public static void SendEmaill(string MailTo, string CC, string BCC, string Subject, string Body)
        {
            //mail message
            MailMessage mM = new MailMessage();

            //Mail Address
            // mM.From = New MailAddress("amermaliksgd@gmail.com")
        ...
        public static void SendEmaill(string MailTo, string CC, string Subject, string Body)
        {
            //mail message
            MailMessage mM = new MailMessage();
        ...
        public static string SendSMS(string CellnoTo, string SmsBody)
        {
            WebClient myWebClient = new WebClient();
            string SPort = "80";
            string Url = "http://websms.eocean.pk";
            string Username = "rts";
            string Password = "2c0a4adf28d925a104e0bc48c46740ed";
            string Recipient = CellnoTo;
            //Recipient of the message if multiple seperate them with ;
            string MessageData = SmsBody;
            //Message text we want to send

            string createdURL = Url + ":" + SPort + "/api/sendsms.php?username=" + Username + "&secret=" + Password + "&to=" + Recipient + "&text=" + MessageData;
            return myWebClient.DownloadString(createdURL);
        }
        ...
        public static void Restore(SqlConnection sqlcon, string DatabaseFullPath, string backUpPath)
        {
            using (sqlcon)
            {
        ...
        //    {
        //        string backupDIR = HttpRequest.MapPath("~") + "\\Backups";
        ...
        //        try
        //        {
        //            string bName = DateTime.Now.ToString("ddMMyyyy_HHmmss");
        //            SqlHelper.ExecuteNonQuery(SqlHelper.GetCon(), CommandType.Text, "backup database MedicMis to disk='" + backupDIR + "\\" + bName + ".Bak'");
        //        }
        //        catch (Exception)
        //        {
        //            throw;
