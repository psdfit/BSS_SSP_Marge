...
{
    public class SRVAMSReportService : ISRVAMSReportService
    {
        ...
        public SRVAMSReportService(IDapperConfig dapper){
            _dapper = dapper;
        }
        public async Task<AMSFormThreeViewModel> GetAMSFormThreeReport(int? schemeId, int? tspId, int? classId, string dateTime)
        {
            try
            {
                ...
                {
                    for (int i = 0; i < traineesCountList.Count(); i++)
                    {
        ...
        public async Task<FormFourModel> GetFormFourReport(int? schemeId, int? tspId, int? classId, string dateTime)
        {
            try
            {
                DateTime? date = null;
                ...
                tspId = tspId > 0 ? tspId : 0;
                classId = classId > 0 ? classId : 0;
                var list = await _dapper.QueryAsync<Details>("dbo.RD_FormFour", new { @SchemeID = schemeId, @TSPID = tspId, @ClassID = classId, @Month = date }, CommandType.StoredProcedure).ConfigureAwait(true);
                FormFourModel model = new FormFourModel();
                model.Details = new List<Details>();
                model.Total = new Total();
                if (list.Any())
                {
                    foreach (var item in list)
                    {
                        ...
                        if (Remarks != null)
                        {
                            string[] spliteDataOnComma = Remarks.Split(",");
                            Remarks = "";
                            model.Total.TotalMajor += item.Major;
                            model.Total.TotalMinor += item.Minor;
                            model.Total.TotalSerious += item.Serious;
                            model.Total.TotalObservation += item.Observation;
                            model.Total.TotalTotal += item.Total;
                        }
                    }
                    model.Details = list.ToList();
                }
                return model;
            }
            catch (Exception ex)
            {
                return new FormFourModel();
            }
        }

        public async Task<ProfileVerificationModel> GetProfileVerificationReport(int? schemeId, int? tspId, int? classId, string dateTime)
        {
            try
            {
                DateTime? date = null;
                ...
                var list = await _dapper.QueryAsync<PVMList>("dbo.RD_ProfileVerification", new { @SchemeID = schemeId, @TSPID = tspId, @ClassID = classId, @Month = date }, CommandType.StoredProcedure).ConfigureAwait(true);
                ProfileVerificationModel model = new ProfileVerificationModel();
                model.PVMList = new List<PVMList>();
                if (list.Any())
                {
                    if (list.ToList()[0].IsLock.Contains("n"))
                    {   
                        foreach (var item in list)
                        {
                            if (item.AttendanceStatus == "present")
                            {
                                if (item.VerificationStatus != "fake")
                                {
                                    ...
                                    if (item.VerificationStatus == "not_verified")
                                    {
                                        item.Remarks = "The trainee was present but reported as Not Verified by field monitor";
                                    }
                                    else
                                    {
                                        item.Remarks = "The trainee was present on the date of visit";
                                    }
                                }
                                else
                                {
                                    ...
                                    if (item.VerificationStatus == "fake")
                                    {
                                        item.Remarks = "The trainee was present but was reported Fake by field monitor";
                                    }
                                }
                            }
                            else
                            {
                                ...
                                if (item.AttendanceStatus == "blank_space" && item.IsPresent == "y")
                                {

                                    if (item.VerificationStatus == "fake")
                                    {
                                        item.Status = "Not Verified";
                                        item.Remarks = "The trainee was present but fake and there was blank space against this trainee";
                                    }
                                    else
                                    {
                                        item.Status = "Verified";
                                        item.Remarks = "The trainee was present but there was blank space against this trainee";
                                    }
                                }
                                else
                                {
                                    ...
                                    if (item.VerificationStatus == "fake")
                                    {
                                        item.Remarks = "The trainee was absent, fake and there was blank space against this trainee";
                                    }
                                    else
                                    {
                                        item.Remarks = statusValues[item.AttendanceStatus];
                                    }
                                }
                            }
                        }
                    }
                    model.PVMList = list.ToList();
                    model.PVMList[0].TraineesPerClass = model.PVMList.Count();
                }
                return model;
            }
            catch (Exception ex)
