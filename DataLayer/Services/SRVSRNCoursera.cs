...
namespace DataLayer.Services
{
    public class SRVSRNCoursera : ISRVSRNCoursera
    {
        ...
        public DataTable GenerateSRNOJT(QueryFilters model, out string IsGenerated)
        {
            ...
            param[0] = new SqlParameter("@SchemeID", model.SchemeID);
            param[1] = new SqlParameter("@Month", model.Month);
            DataSet ds = SqlHelper.ExecuteDataset(SqlHelper.GetCon(), CommandType.StoredProcedure, "dbo.GenerateOJTSRN", param.ToArray());
            IsGenerated = ds.Tables[0].Rows[0]["SRNGenerated"].ToString();
            if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                return ds.Tables[0];
            return null;
        ...
        public DataTable FetchOJT(QueryFilters model)
        {
            ...
            param[0] = new SqlParameter("@SchemeID", model.SchemeID);
            param[1] = new SqlParameter("@Month", model.Month);
            DataTable dt = SqlHelper.ExecuteDataset(SqlHelper.GetCon(), CommandType.StoredProcedure, "RD_OJTSRN", param).Tables[0];
            return dt;
        }
    }
}
